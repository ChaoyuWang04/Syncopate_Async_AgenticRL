...
def write_split_checked(out_dir: Path, train_rows: list[dict[str, Any]], val_rows: list[dict[str, Any]],
    ...
    _write_parquet(out_dir / "train.parquet", train_rows)
    _write_parquet(out_dir / "val.parquet", val_rows)
    iso = {"checked_by": "syncopate.pipeline.split.assert_split_isolation", "pool": pool, "split_dir": str(split_dir),
           "train_counts": rep_t["counts"], "val_counts": rep_v["counts"], "offenders": 0}
    man = {"isolation": iso, "train_count": len(train_rows), "val_count": len(val_rows), **(manifest_extra or {})}
    (out_dir / "manifest.json").write_text(json.dumps(man, ensure_ascii=False, indent=1, sort_keys=True, default=str) + "\n", encoding="utf-8")
    return man
