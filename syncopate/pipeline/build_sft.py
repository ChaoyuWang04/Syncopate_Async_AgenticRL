...
"""v16 · SFT 训练集建库（runbook `scripts/v16_pipeline.sh sft-data` 的执行件）。
    ...
    U_BUILD_DRY=6 python -m syncopate.pipeline.build_sft   # 本机结构演练（不调教师、"[DRY" 占位、产物落 _audit/<DV>/dry/）
...
桶：压舱（当前切分 SFT 桶）· L2 多轮读数 · L1 定义 · 闲聊 · 六族派生行（syncopate.pipeline.multiturn）· CoT 蒸馏；闸表与来历见 26 §W4′。
...
# MIN_FIELDS 唯一定义在 syncopate.pipeline.multiturn（09-02 收口副本；两处各写一份会漂）
from syncopate.pipeline.multiturn import MIN_FIELDS
...
from syncopate.pipeline.multiturn import (
    DRY,
    EXAM_FILES,
    answer_turn,
    as_multiturn,
...
for _fn in EXAM_FILES:          # 与 syncopate.pipeline.multiturn 同一份清单
...
# ★ 桶数量下限（唯一定义；gate() 与 syncopate/pipeline/supply_gate.py 两边 import 同一份。09-04 实案：供给脚本抠源码正则，
...
async def gen_cot(client, tok, max_rows=100) -> list[dict]:
    from syncopate.evaluation.value_match import gold_values
...
async def build_l2_l1(tokenizer, registry, client):
    ...
    from syncopate.pipeline.multiturn import campaign_of as _cof
    ...
    if not DRY:
        ...
        from syncopate.pipeline.multiturn import BALLAST_REPLIES as _BR
...
async def _replay_frozen(tokenizer, registry, parquet_path: str, base_index: int,
    ...
    ★ 冻结的是**语义**不是字节：换壳之后逐字节冻结在物理上不可能，
      ...
      —— 全量 419 条已由旧版迁移检查器证过（证据与脚本见 25 §R2①及 archive）。
    """
    from syncopate.pipeline.build_dataset import build_sft_row
    from syncopate.pipeline.split import load_split_bundles
...
async def main() -> int:
    ...
    if DRY:
        ...
        from syncopate.pipeline.multiturn import SOURCE_OF as _SRC
        ...
        _dry.to_parquet(_dp)
        print(f"[DRY] 演练产物 → {_dp}（给 syncopate/pipeline/data_gallery.py 看终态）· 不提前返回，继续走全部结构闸")
    ...
    from syncopate.pipeline.data_audit import audit as _audit
    if DRY:
