...
"""管线前提检查 —— 「两个东西应当相同 / 某集合应当完整」这类断言的集中执行处。
    ...
    python -m syncopate.pipeline.invariants                     # 全跑
    python -m syncopate.pipeline.invariants --only merge rank   # 只跑某几组
...
def _active_shell_scripts() -> list[Path]:
    """Return runnable shell scripts, excluding frozen history."""
    return sorted(
...
def _supply_vs_floors(log):
    ...
    r = subprocess.run(
        [_sys.executable, "-m", "syncopate.pipeline.supply_gate"],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
...
QUARANTINE_DOC = "docs/archive/syncopate/pre-consolidation-v16/21-invalidated-numbers.md"
...
def _quarantined_tokens() -> list[tuple[str, str]]:
    """从归档 21 号文档的 ```quarantine``` 块里读作废清单 —— **唯一来源**。
...
def _no_script_overrides_contract(log):
    ...
    offenders, waived = [], []
    for sh in _active_shell_scripts():
    ...
    if offenders:
        ...
        log(f"  ✅ 扫了 {len(_active_shell_scripts())} 个脚本，"
            f"没有一处显式传契约参数")
    if waived:
...
def _rank_readers_have_guard(log):
    ...
    targets = {
        "syncopate/train/ckpt_to_adapter.py": True,
        "scripts/tools/rl_ckpt_drift.py": True,
        "syncopate/train/prune_ckpts.py": True,
    }
    ok = True
    for rel in targets:
        p = ROOT / rel
