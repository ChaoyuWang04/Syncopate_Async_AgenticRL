"""★ 数据门禁：每个大版本重建之前跑一遍，不过就不许开训。
    ...
    python -m syncopate.pipeline.data_gates --batch data/batches/v13 --split-dir data/splits/v13
    python -m syncopate.pipeline.data_gates --batch data/batches/v13 --verbose        # 打全表

两组判据，性质不同：
    **D1–D11 多样性**（只要 --batch）    见 docs/syncopate/03-DATA.md
    **L1–L2  泄露**（要 --split-dir）    见 docs/syncopate/03-DATA.md
