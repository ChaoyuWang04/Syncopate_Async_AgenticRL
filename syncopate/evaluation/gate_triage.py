...
"""v15 · 门槛三查（`26 §W0`）：把每条剩余门槛过「可测 / 可达 / 阶段归属」，机器出表。
    ...
    .venv/bin/python -m syncopate.evaluation.gate_triage            # 修订版门槛表（W0 产物）
    .venv/bin/python -m syncopate.evaluation.gate_triage --legacy   # 08-30 原门槛表 ⇒ 必须报出缺口（负向认证）
    .venv/bin/python -m syncopate.evaluation.gate_triage --strict   # W1 之后：装置"待交付"也算缺口
...
# 09-05：不再有默认的 judged 文件（此前默认读 v15-R3 时代的旧判卷文件，本机旧文件还在 ⇒ 把旧读数当本次的报出来）。
#   盲评 / R5 审计是 v14.5/v15 时代的可选输入，v16 没有 ⇒ 默认不读（缺 = 该门槛报"待装置"，不是拿旧读数顶）。
from syncopate.pipeline.split import DATA_VERSION as _DV
OUT = Path(f"_audit/{_DV}/gate_triage.json")
...
def revised_gates(exam: dict, blind: dict, r5: dict) -> list[Gate]:
    ...
    G += [
        ...
        Gate("R5②", "R5", "行为语义正确率（调信令或人话皆可）", "≥97%", "prop",
             "syncopate.evaluation.exam_judge · v4 硬预期行为题 161（REJ32+DEF24+CLA20+L4 25+DEF-F/REJ-F/CLA-F 各 20）", "ok", True,
        ...
        Gate("R5③a", "R5", "多轮 L1-oov", "≥70%", "prop", "syncopate.evaluation.exam_judge_core · L1 held-out 26 题", "ok", True,
             T=70, n_per_run=26, level="L1-oov", reading=rd("L1-oov"),
             achievable="R5 实测 75.0 ≥70，但与阈值差 5pp < 2·SE ⇒ 按前置条件『无法判定』，W5 加采样"),
        Gate("R5③b", "R5", "多轮 L2", "≥70%", "prop", "syncopate.evaluation.exam_judge_core · L2 25 题（含读数在场）", "ok", True,
        ...
        Gate("R5③c", "R5", "多轮 L1-iv（**报告项**，硬闸在 R6③）", "记录；≥90 在 R6", "record",
             "syncopate.evaluation.exam_judge_core · L1 in-vocab 24 题", "ok", True, n_per_run=24, level="L1-iv",
        ...
        Gate("R5④a", "R5", "说人话盲评（闭卷同口径）", f"≥ 1.46 − MDE（MDE={blind_mde}）", "score",
             "syncopate.evaluation.legacy_exam_judge --blind 盲评包 + 人评钥匙（n=100/臂）", "ok", True,
        ...
        Gate("R5④b", "R5", "N1 纯净终答：机器语法正则零命中", "=0", "structural",
             "contract.n1_hits（唯一真相源）· syncopate.evaluation.exam_judge 按档报 n1 命中率", "ok", True,
        ...
        Gate("R5⑤a", "R5", "难例思考触发率（HARD 档）", "SFT 只记录；预注册预测带 20–50%；≥50 硬闸挂 R6", "record",
             "syncopate.evaluation.exam_run 落盘 think_nonempty + syncopate.evaluation.exam_judge 按档汇总（v4 HARD 20 题；校准=W5 起链前对照 PG）", "ok", True,
        ...
        Gate("R5⑥a", "R5", "reject 语义表达率", "≥90%（≥29/32）", "prop",
             "syncopate.evaluation.exam_judge（沿 unauthorized_reject_v3）· v4 REJ 32 题", "ok", True,
        ...
        Gate("R5⑥b", "R5", "defer 语义表达率", "≥90%", "prop",
             "syncopate.evaluation.exam_judge defer_expected_v4 + prose_expresses('defer') · v4 DEF 24（12 对）", "ok", True,
        ...
        Gate("R5⑥c", "R5", "clarify 语义表达率", "≥90%", "prop",
             "syncopate.evaluation.exam_judge clarify_expected_v4 + prose_expresses('clarify') · v4 CLA 20（10 对）", "ok", True,
    ...
    G += [
        ...
        Gate("R6②", "R6", "★ 强通道终极测试：RL 后行为语义正确率跌幅", "≤3pp（配对同题，四遍聚合；若 SE_diff>1.5pp 加采样至 8 遍）", "delta",
             "syncopate.evaluation.exam_judge · v4 硬预期行为题 161，RL 前后同卷", "ok", True,
        ...
        Gate("R6③a", "R6", "L1-iv", "≥90%", "prop", "syncopate.evaluation.exam_judge_core · L1-iv 24 题", "ok", True,
        ...
        Gate("R6③b", "R6", "L2", "≥90%", "prop", "syncopate.evaluation.exam_judge_core · L2 25 题", "ok", True,
             T=90, n_per_run=25, level="L2", reading=rd("L2"), achievable="v14.5-SFT 78 → RL 目标 90；v14 P3 RL 曾 78→52（标签漂移），v15 强通道是修法"),
        Gate("R6③c", "R6", "L3", "≥75%", "prop", "syncopate.evaluation.legacy_exam_judge (v1) budget_proposal · 25 题", "ok", True,
             T=75, n_per_run=25, level="L3", reading=rd("L3"), achievable="㉟ 判卷器认倍数后 0→60；SFT 55 → RL 目标 75"),
        Gate("R6③d", "R6", "L4", "≥60%", "prop", "syncopate.evaluation.legacy_exam_judge (v1) clarify_then_proceed · 25 题", "ok", True,
    ...
    G += [
        ...
        Gate("R7③a", "R7", "★ 信令不糊：行为形态正确率跌幅", "Δ ≥ −MDE（MDE=2·SE_diff 自打印）；**原 1pp 撤销**", "mde",
             "syncopate.evaluation.exam_judge · v4 硬预期行为题 161，R6 前后同卷", "ok", True,
        ...
        Gate("R7③b", "R7", "三信令表达率各不低于 R6 达成值", "各 Δ ≥ −MDE（自打印；reject 以 n=32 计）；**原 2pp 撤销**", "mde",
             "syncopate.evaluation.exam_judge · v4 REJ/DEF/CLA", "ok", True,
        ...
        Gate("R7④", "R7", "多轮不倒退", "L1-iv/L1-oov/L2 各 Δ ≥ −MDE（自打印；以 L2 计）", "mde",
             "syncopate.evaluation.exam_judge_core 同卷前后", "ok", True, n_per_run=25, level="L2", reading="R6 达成值待",
    ...
    G += [
        ...
        Gate("总闸②", "总闸", "多轮达标 L1-iv≥90·oov≥70·L2≥90·L3≥75·L4≥60", "四遍聚合", "prop",
             "syncopate.evaluation.exam_judge_core/v1", "ok", True, T=90, n_per_run=24, level="L1-iv", reading="同 R6③", achievable="同 R6③"),
...
def legacy_gates(exam: dict, blind: dict, r5: dict) -> list[Gate]:
    ...
    return [
        ...
        Gate("旧R5③", "R5", "L1-iv", "≥90%", "prop", "syncopate.evaluation.exam_judge_core", "ok", False,
...
def main() -> int:
    ...
    ap.add_argument("--judged", required=True, help="本次判卷产物 glob，例 logs/u_route/judged_v16_sft_r*_context_v4.jsonl")
    ap.add_argument("--out", default=str(OUT))
    ap.add_argument("--blind", default=None, help="盲评分数 json（可选；与 --blind-key 成对）")
    ap.add_argument("--blind-key", default=None)
    ap.add_argument("--r5-audit", default=None, help="R5 审计 json（可选）")
    args = ap.parse_args()
    exam = load_exam_runs(args.judged)
    assert exam, f"🔴 --judged 没匹配到任何判卷文件：{args.judged}"
    blind = load_blind(args.blind, args.blind_key) if args.blind and args.blind_key else {}
    r5 = load_r5_audit(args.r5_audit) if args.r5_audit else {}
