...
逐轮收集 行为/回复/工具调用/审批提案，落机读原始结果。
    ...
    .venv/bin/python -m syncopate.evaluation.exam_run --exam context --arm cand
    .venv/bin/python -m syncopate.evaluation.exam_run --exam talk --arm base --concurrency 4
...
def resolved_behavior(behavior: str | None, *, status: str,
                      proposal: object | None) -> str | None:
    """Fill the one legitimate behavior-less terminal shape.

    A model write call is converted by the action gate into an approval proposal.
    The run then parks at ``waiting_for_user`` before ``finish_run`` writes a model
    result, so its terminal payload has no behavior even though the observed model
    behavior is unambiguously ``tool_call``.  Runtime failures/cancellations remain
    unset and are still rejected by the Exam evidence gate.
    """
    if behavior:
        return behavior
    if status == "waiting_for_user" and proposal is not None:
        return "tool_call"
    return behavior
...
async def run_turn(c: httpx.AsyncClient, pg: asyncpg.Pool, cid: str, msg: str) -> dict:
    ...
    if sig_ev is not None:
        ...
        extra = {}
    beh = resolved_behavior(beh, status=st, proposal=prop)
    return {
