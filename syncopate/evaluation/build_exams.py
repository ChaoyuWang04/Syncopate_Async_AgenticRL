...
"""v15 · exam_v4 生成器（`26 §W1` 科目表；生成后**冻结**，改内容=新版本号）。
    ...
    .venv/bin/python -m syncopate.evaluation.build_exams   # → data/u_route/context_v4_exam.jsonl
...
★ 脚本化历史（`prior`）：多轮题的"上一轮"由考卷**直接写进会话历史**（syncopate.evaluation.exam_run 按线上同一张
