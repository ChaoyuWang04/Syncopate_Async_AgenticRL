...
"""SFT 选点：按正确的尺子排序候选，**并把删除做成流程的收尾动作**。
    ...
    python -m syncopate.train.select_sft_ckpt checkpoints/sft/v14              # 只排序，不删
    python -m syncopate.train.select_sft_ckpt checkpoints/sft/v14 --keep epoch1 --prune
...
def _metric(kind: str, cand_dir: Path, audit_dir: Path | None = None) -> dict | None:
    ...
    **v3 时代**的 `M1_ctrl_epoch1.json`，并打出 `熵 0.483 / 有梯度 44` ——
    ...
    source = audit_dir or (ROOT / "_audit")
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--audit-dir", type=Path, default=None,
                    help="只从这个目录读取本轮 entropy/eval 审计；固定管线必须显式传逐 run 目录，避免撞到旧跑")
    args = ap.parse_args(argv)

    out = args.out_dir if args.out_dir.is_absolute() else ROOT / args.out_dir
    audit_dir = None if args.audit_dir is None else (
        args.audit_dir if args.audit_dir.is_absolute() else ROOT / args.audit_dir)
    ...
    for d in cands:
        ...
        ent = _metric("entropy", d, audit_dir)
        e = ent.get("decision_mean_entropy") if ent else None
        ev = _metric("eval", d, audit_dir)
