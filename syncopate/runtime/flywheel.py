...
async def build_case_from_run(db: Any, *, org_id: str, run_id: str) -> dict[str, Any]:
    ...
    prior 形状 = agent_runs 行（user/status/error/result），`syncopate.evaluation.exam_run.seed_prior` 直接吃。"""
    ...
    case = {
        ...
        "judge": {"type": None},                          # 由人从 syncopate.evaluation.exam_judge 现有判类里选；空 = 报告项
