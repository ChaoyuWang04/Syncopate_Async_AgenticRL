...
课件说的 `sandbox_refund_ledger` 里有 `idem_key = "R1-step7"` —— **代码里没有这个字段**。这和 [07-sandbox-environment](07-sandbox-environment.md) §2.3 的结论一致：**整套代码没有 idempotency 概念**。

同样地，[06-verifier-and-reward](06-verifier-and-reward.md) §3.1 已确认 `verifier_input_fact` / `policy_matched` / `risk_checked` / `evidence_attached` 四个字段里只有 `refund_issued` 真实存在。
...
原因很清楚：**近一半的 case 根本不需要写动作**。它们是 inform / deny / clarify 类——查清楚了直接回复客户。这和 verifier 的 `calculate_outcome` 三分支设计完全吻合（[06-verifier-and-reward](06-verifier-and-reward.md) §2.3）：
...
- parse_error 重试（gold 里 **0 次**，见 [05-anatomy-of-a-trajectory](05-anatomy-of-a-trajectory.md) §2.4）
- 调错工具、走弯路
...
1. **caps 才是拉开 reward 差距的东西。** 子分加权和最多把 raw 拉到 [0,1] 连续变化；而 cap 一命中直接把最终 reward 摁到 0.25–0.55（[06-verifier-and-reward](06-verifier-and-reward.md) §4）。**归因 cap = 归因了 reward 的主要方差来源。**
...
它的完整链路（见 [02-train-inference-mismatch](../../infra_exp/legacy-notes/02-train-inference-mismatch.md) §4）：
```
...
| 方案 | 改动 | 风险 |
|---|---|---|
| **A. 复用 `rollout_is_weights`** | 在 `compute_rollout_correction_and_add_to_batch` 之后把 `c_t` 乘进去 | 🔴 **和 TIS 语义冲突**——它已经承载了重要性采样权重，两者相乘会污染 `rollout_corr/*` 全部诊断指标，也破坏 [23-research-question](../pre-consolidation-v16/23-research-question.md) 的 ESS 分析 |
...
**步级信用分配会加剧 sequence-level TIS 的问题。** 如果 `c_t` 让不同步的 token 权重差异变大，而 [23-research-question](../pre-consolidation-v16/23-research-question.md) §8 的 `S = Σδ_t` 是无权求和——**两者对"一条轨迹内 token 不等价"这件事的处理是不一致的**。真要同时上，得先想清楚 TIS 的求和是否也该加权。
