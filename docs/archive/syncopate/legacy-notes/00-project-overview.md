...
4. ~~**rollout correction（TIS）细节**~~ → 已完成精读，独立成篇：[02-train-inference-mismatch](../../infra_exp/legacy-notes/02-train-inference-mismatch.md)。
...
8. **`rollout_is=sequence` 在 8 轮 multi-turn 下是否合适**：`masked_sum` 会把全轨迹所有模型 token 的 log_ratio 相加，序列越长越容易撞上截断上界。见 [02-train-inference-mismatch](../../infra_exp/legacy-notes/02-train-inference-mismatch.md) §6。
