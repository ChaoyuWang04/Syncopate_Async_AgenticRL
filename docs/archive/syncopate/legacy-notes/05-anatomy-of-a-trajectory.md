...
- 对 [02-train-inference-mismatch](../../infra_exp/legacy-notes/02-train-inference-mismatch.md) 的 TIS：`masked_sum` 求 `S = Σδ_t` 时只累加这 39%，所以 **T ≈ 222（P50）而不是 538**。这个数比我上次用字符估的 280 低 21%，需要修正 [23-research-question](../pre-consolidation-v16/23-research-question.md) §3 的表格。
...
因为 prompt 里 81% 是完全相同的工具 schema。**这对 remove padding 的收益判断是决定性的**（见 [04-remove-padding](../../infra_exp/legacy-notes/04-remove-padding.md) §4.2）：填充浪费**全部来自 response 侧**，prompt 侧几乎没有可省的。按 `max_response_length=2048` 算，gold 轨迹的填充率只有 538/2048 = 26%——**remove padding 在这个任务上能省的主要就是这块**。
...
含义：
- **KV cache 的 prefix 复用潜力极大**——5417 个 token 的前缀在所有样本间完全一致。vLLM 的 automatic prefix caching 理论上能省掉几乎全部 prefill。但注意 [03-resource-scheduling](../../infra_exp/legacy-notes/03-resource-scheduling.md) §3.2 说过，每次 wake_up 后会 `reset_prefix_cache`（权重变了旧 KV 必须作废），**所以 colocate 模式下这个红利每个参数版本只能吃一轮**。异步分离模式下 rollout 权重更新更稀疏，反而更能吃到 prefix cache——**这是异步化的一个此前没被提及的额外收益，Phase 2 值得单独测**。
...
**要改的地方**：[23-research-question](../pre-consolidation-v16/23-research-question.md) §3 的安全边界表用的是 T=300/800，实测应改成 **T=222(P50) / 381(P90) / 534(max)**。方向不变（同步 colocate 下安全），但边界更宽松一点。
...
2. **thinking token 占 `max_response_length` 预算，而且拿梯度。** gold 轨迹 response 区 P50 才 538 token；真实 RL rollout 里 thinking 很可能是 response 长度的主要来源，**也是长尾的主要来源**。这直接影响 [23-research-question](../pre-consolidation-v16/23-research-question.md) 的核心变量 T ——**T 的实际分布可能远比 gold 数据宽**。
