...
> 📦 **历史研究问题，当前没有进入主线任务队列。** 当前训练任务见
> [docs/syncopate/01-TASKS.md](../../../syncopate/01-TASKS.md)。

> 立项日期：2026-07-28
> 状态：**假设已成型，待实证**。理论推导见 [02-train-inference-mismatch](../../infra_exp/legacy-notes/02-train-inference-mismatch.md) §8。
...
推导过程见 [02-train-inference-mismatch](../../infra_exp/legacy-notes/02-train-inference-mismatch.md) §8.2。假设 δ_t i.i.d.、均值 μ 方差 σ²：
...
> T 已用真实 Qwen3 tokenizer 在 121 条 gold 轨迹上实测（见 [05-anatomy-of-a-trajectory](../legacy-notes/05-anatomy-of-a-trajectory.md) §2.1），
...
→ **本实验必须全程强制 `bypass_mode=false`**，否则 sequence vs token 的对比无从谈起。详见 [03-resource-scheduling](../../infra_exp/legacy-notes/03-resource-scheduling.md) §4.4。
...
已产出第一个点 σ²(0) ≈ 2.0e-4/token。两条路的分工见 §9.9。

### 9.9 离线合成 vs 真异步：k 精确可控，两条路互补（2026-08-19 自 06 §4.1 迁入）

要量 $\sigma^2(k)$（核心量 $\mathrm{ESS}/N \approx \exp(-T\,\sigma^2(k))$）**并不需要真异步**——
异步只是产生 staleness 的一种方式。直接做法：第 $t-k$ 步 policy 生成的轨迹，
用第 $t$ 步 policy 重算 logprob ⇒ ratio 分布 ⇒ ESS；$k$ 由我们给定。

| | 真异步 | 离线合成（`train/staleness.py`） |
|---|---|---|
| $k$ 取值 | 随机、由调度决定（M7 实测 k 根本没长出来） | **精确可控**，能扫 k=0,1,2,4,8 |
| 硬件 | ≥2 GPU（已打通） | 单卡即可 |
| σ²(k) 曲线 | 能，含混杂且碰运气 | **能，且干净** |
| 吞吐/空转 · 分布漂移 | ✅ | ❌ |

⇒ H1/H2 核心曲线用离线合成扫；吞吐与漂移用真异步补 ——
但**真异步要先让轨迹真的跨越版本边界**（调大 `--latency-scale` 或缩短同步周期），
否则量到的只会是又一个 0（M7 的 `partial_ratio=0` 教训，§9.8）。
已有第一个实测点：σ²(0) ≈ 2.0e-4/token（ESS/N=0.846, T≈825）。
