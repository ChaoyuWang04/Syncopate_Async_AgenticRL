...
> ⚠️ 证据列里的 `schema.sql:行号` 是 K0 当天的定位；该文件 K2 起已退役，内容整份内嵌在
> `migrations/versions/0001_baseline.py`，按那里查。带「09-02」的状态是施工后的实况。
...
⚠️ 首次跑 `test_retrieval` 3 红 = 语料未灌（08 §1 干净机器缺口，`syncopate/runtime/ingest_corpus.py` 后消失）；
...
| E6 | 九条 SLO 自动读数 | ✅ 09-02（scripts/serving/slo_readout.py；P95 需 --api） | `metrics.py:58-137` 四项（按意图延迟/排队/工具延迟/读写分桶）+ loadtest §19 判定 | 九条对齐 → K9-1 |
...
| E19 | 备份 RPO/RTO + 恢复演练 | 🔶 | `scripts/serving/dr_drill.sh` 数据层重建 RTO 1.5s（K11-4）；生产备份策略与权重那半挂账（30 §1.5 D2/D3） | 灰测放真人前 |
...
| 压测 | 容量与劣化 | `scripts/serving/runtime_loadtest.py`（B-6；Celery 化后需重测 S-05） |
...
**底座（本机 2026-09-02）**：PostgreSQL **16.15**（conda-forge，用户态，`~/.local/share/syncopate/pgdata/16`）· asyncpg 裸 SQL（`db.py:29-60`，JSONB/NUMERIC 显式编解码）· 事务 `db.tx()` · 行锁 `FOR UPDATE SKIP LOCKED` 在用 · 触发器在用（`notify_run_event`）· 22 张表（runtime 9 + RAG 2 + 参考数据 10 + conversations）· 行数 0（新库）· Redis **8.10.1**（用户态，`scripts/serving/redis_bootstrap.sh`，requirepass/AOF/noeviction）。训练机：PG 16 deb（`/workspace/tools/postgres`），`max_connections` 曾调 300。
