...
> 📦 **Modal/B200 迁移施工快照，不再是当前入口。** 当前环境见
> [docs/syncopate/05-COMPUTE.md](../../../syncopate/05-COMPUTE.md)，当前训练状态见
...
## 0 · 一句话现场（09-05 run28 全绿：v16 SFT 训练集落地）

**新栈全管线的"机器部分"全部冒烟通过（SFT 机制 / 考场链路 / RL verl 0.9 首次跑通 / OPD 机制）；管线收成唯一入口 `scripts/v16_pipeline.sh`（18 stage，默认值直接跑）；
三桶隔离做成硬机制（守则⑱）；题库扩量到 2030 道新情景；**v16 SFT 训练集 run28 全绿落地（1222 行 / 18 桶，出厂体检零红，云盘 /vol/data/sft/v16）**。
整条管线已逐段审查：v16 主链不再依赖旧版脚本，可复用 Python 组件已归入 `syncopate/`，`scripts/` 只保留入口、编排和探针；同时去掉 v8 审计输入、补上被跳过的泄漏闸、
...
★ 固定管线   bash scripts/v16_pipeline.sh [--dry-run] [--profile smoke|candidate] <stage|all>   # 09-04 起唯一入口（本机 == 云上）
             stages: cases menus split gates supply rl-data teacher sft-data sft-train sft-eval sft-select merge exam rl-train rl-adapter rl-eval opd-train opd-eval
