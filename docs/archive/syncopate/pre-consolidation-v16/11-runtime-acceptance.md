...
| R5.7 | RAG 服务（§36） | ✅ **2026-08-17 已建** | `syncopate/runtime/retrieval.py` + PG 两张表 + `syncopate/runtime/ingest_corpus.py`。**三态契约**（查到 / 查不到 / 查不了）· 生效期与取代关系精确计算 · 多租户作用域 · 11 条测试。⇒ 压测场景④有被测对象了、`retrieval_empty` 有生产者了、§19 的检索 P95 可量了（实测 3ms）。设计见 **`10-rag-retrieval.md` R 部分**（原 12 号已并入） |
...
单流基线（`scripts/serving/measure_tpot.py`，vLLM 0.12 · 1×5090 · 契约采样，vLLM 服务起法见 `09 §0`）：
...
**② ✅ M9.7 压测已跑（2026-08-20，真模型驱动，`scripts/serving/runtime_loadtest.py`）——
