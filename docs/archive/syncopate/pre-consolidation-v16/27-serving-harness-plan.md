...
> 我们系统的一部分。
>
> **状态（2026-09-02）：K0–K11 全部落地，本文已从施工计划书转为施工记录。** 各节头的
> "✅ 落地"注记是当日实况；§14 是收口与全线挂账；§16 是开工前的四件裁定。
> 下一位负责人从 **09 §0.0 K 线交接** 进入，不要从本文第 3 节开始按步骤重做——那些步骤已经做完，
> 步骤里的"现状"描述是开工前的现状，不是现在的。
...
| 阶段 | 对应课件 | 一句话 | 本文状态 |
|---|---|---|---|
| **K0** | — | 现状盘点：49 条能力清单 × 现有实现对照 | ✅ 落地 09-02（§2） |
| **K1** | CH1 | Run API：六入口 · 幂等 · 协作式取消 · 双层错误码 | ✅ 落地 09-02（§3） |
| **K2** | CH2 | 数据库落库：两本账 + 8 表 + 补课件自认的缺口 | ✅ 落地 09-02（§4） |
| **K3** | CH3 | 队列与 worker：Outbox · lease · 五层幂等防线 | ✅ 落地 09-02（§5） |
| **K4** | CH4 | 状态机：白名单 · 唯一迁移入口 · 一事务写全 | ✅ 落地 09-02（§6） |
| **K5** | CH5 | 执行 loop：对照收编 · 意图日志 · 错误分层 | ✅ 落地 09-02（§7） |
| **K6** | CH6 | Tool Runtime：四道闸收编 · 五态 tool_calls · 注册断言 | ✅ 落地 09-02（§8） |
| **K7** | CH7 | SSE 事件流：对照收编 · 事件分层 · 通知唤醒 | ✅ 落地 09-02（§9） |
| **K8** | CH8 | 恢复与重放：sweeper · 对账闭环 · 四动词纪律 | ✅ 落地 09-02（§10） |
| **K9** | CH9 | 生产硬化：SLO · 预算 · 观测三件套 · 变更可逆 | ✅ 落地 09-02（§11） |
| **K10** | CH10 | 回流飞轮：结果信号回流 · 与训练线的接口 | ✅ 落地 09-02（§12） |
| **K11** | 附录 A | 上线总清单 · Runbook · 复盘模板 | ✅ 落地 09-02（§13） |
...
> ✅ **K9 已落地（2026-09-02，裁剪面按取舍表）**：SLO 九条自动读数 `scripts/serving/slo_readout.py`（`metrics.snapshot` +
...
> ✅ 落地 2026-09-02：交付物 = `30-serving-release-checklist-and-runbook.md`（清单 42 条：✓36 · ✓/挂账 3 · 挂账 3；
> runbook 六张卡 + 传导图；复盘模板回填 08-20 事故；灾备演练 `scripts/serving/dr_drill.sh` RTO 1.5s；SLO 基线
> `_audit/serving_k11/slo_baseline_2026-09-02.txt`）。卡片第一批查询由 `scripts/serving/runbook_queries.py` 逐条真跑。
> 门槛 ①–⑥ 全过；K11-4 只演练了数据层那半（模型权重本机没有，挂账到回训练机）。
...
**K0–K11 十二个阶段全部成稿（2026-09-01）→ 全部落地（2026-09-02，单日，本机无 GPU）。**
粗估 18 个工作日实际一天：差额来自准则〇（77 条能力里 K0 前已有 ~55 条，施工是收编不是重写）
与门槛先行（每阶段先写负向认证再补机制）。每阶段的 ✅ 落地注记在各节头；对照清单在 29；
中间件坑与优化候选在 28（C-15 负向认证 ⬜、O-01..O-10 待做，是下一轮的料）。

**全线挂账（都有归属与复活条件，详见 30 §1）**：V4 事件 payload 版本号 · D3 生产备份策略 ·
C5 API/写工具并发限流 · T5 压测 Celery 化后重测（S-05，需训练机端点）· 停队列/回滚只写命令未真演练 ·
前端 👍👎 入口与三处改名未 build（本机无 node）· 灾备的模型权重那半。
