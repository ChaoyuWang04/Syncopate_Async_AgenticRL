...
> 📦 **历史审计快照。** 当前数据和训练不变量分别见
> [03-DATA](../../../syncopate/03-DATA.md) 与 [04-TRAINING](../../../syncopate/04-TRAINING.md)。
...
✅ syncopate/pipeline/invariants.py：把上面这些做成可重复跑的检查（见 §11）
...
```python
# syncopate/train/prune_ckpts.py:47
...
| Q5 | 🟠 SFT 与 RL 看到的 token 序列同构（整段渲染 vs 增量拼接） | ✅ **探针已落地**：`scripts/v16/probe_sft_rl_consistency.py`（parquet vs 当前回放逐 token 比）。⚠️ 首跑就抓到 §12 那个真问题 | CPU · 抽样分钟级 |
...
| `syncopate/train/ckpt_to_adapter.py` | 原来那句手写断言（就是它炸出 E21）改调共用函数 |
| `scripts/tools/rl_ckpt_drift.py` | 🆕 加断言（此前**没有**，一直静默只读 rank_0） |
| `syncopate/train/prune_ckpts.py` | 🆕 加断言 + `next(glob)` → `sorted(glob)[0]`（此前**非确定性**地留一份） |
| `syncopate/train/launch_rl.py` | 🆕 `--model` 目录里有 `lora_adapter/` ⇒ 报错 |
| `syncopate/train/merge_adapter.py` | 🆕 合并后校验保真残差，超阈值报错 |
| 🆕 `syncopate/pipeline/invariants.py` | 8 条检查，分 4 组（merge / rank / audit / rollout） |
...
**发现路径**：给 Q5/Q6 写探针（`scripts/v16/probe_sft_rl_consistency.py`）→ 首跑 6 条
