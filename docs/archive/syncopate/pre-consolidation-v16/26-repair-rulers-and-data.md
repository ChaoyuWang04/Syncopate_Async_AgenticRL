...
> 📦 **v15/v16 施工与证据快照，不代表当前任务顺序。** 当前说明见
> [03-DATA](../../../syncopate/03-DATA.md)、[04-TRAINING](../../../syncopate/04-TRAINING.md) 和
...
|---|---|---|---|---|
| 1 | 历史的位置 | 折成文本塞进 `Case.user_message`：`"[上一轮] 用户：…\n[上一轮] 助手：…"`（`syncopate/pipeline/build_sft.py:609-610,686-693`）；结构根因=`Case` 只有一个 `user_message` 字段（`syncopate/core/schemas.py:76`） | 真消息对（`decider.py:152-190`） | W2 |
...
| 6 | 工具菜单 | 按模板族裁剪（`syncopate/pipeline/tool_menus.py`；v8 时代 docstring 记 8–13，文档记 16–17，代码出处查不到，§7） | 全量 34 | W2（训练改全量 34，已裁定） |
...
⑦ v1 判卷器仍截断失败列表 fails[:20]（syncopate/evaluation/legacy_exam_judge.py:145）——v2 修了 v1 没跟 → W1⑤
...
```
产物   syncopate/evaluation/gate_triage.py（三查机器出表，只读落盘文件）· _audit/v15_w0/gate_triage.json
...
⑪ 收尾：syncopate/evaluation/gate_triage.py 登记表按 v4 实际题数回填（硬预期行为题 = REJ+DEF+CLA+L4+
...
```
产物   syncopate/evaluation/build_exams.py → data/u_route/context_v4_exam.jsonl（361 题：L1 50·L2 25·L3 25·L4 25
       逐字继承 · REJ 32 · DEF 24 · CLA 20 · HARD 20 · DEF-F 20 · REJ-F 20 · CLA-F 20 · L2-x 20 · WIN 20 ·
       META/PRG/COR/TIME 各 10 报告项）· syncopate/evaluation/exam_judge.py（13 个新判类 + 按档读数：思考率/N1/
       编造/写操作）· syncopate/evaluation/exam_certify.py · v16_exam_run 加脚本化历史 + think_nonempty ·
...
⑤ 菜单（#6，已裁定=训练改全量 34）：先实测全量菜单的 token 占用与现契约 prompt 预算；
   ...
   ★ 09-02 实测（scripts/archive/pre_v16_mainline/v15_w2_menu_budget.py → _audit/v15_w2/menu_budget_v13.json）：
     修剪前 工具块 6534 tok · 全量菜单最长 prompt 8095（上限 5760 / 真实约束 6144=14336−8192）
     修剪后 工具块 5468 · 最长 7167（scripts/archive/pre_v16_mainline/v15_w2_trim_tool_desc.py：34 条描述按「只留做什么/输入/
...
代码   syncopate/core/prior_turns.py（历史→消息对的唯一渲染，decider 与 rollout_loop 共用）·
       ...
       syncopate/pipeline/multiturn.py（prod_context / as_multiturn / 六族第一波训练行 / 同形体检）·
...
代码   ① think 做轻：gen_cot_v15 的 one_think max_tokens 900→350 + step_sample 画像闸（≤350 字·≤2 段·中文≥0.5），
       ...
       ② 触发显性化：syncopate/pipeline/cot_prompt.py explicit_hard_prompt（4 前缀/4 后缀多步诊断问法，按 case_id 确定性选）
         接进 gen_cot_v15._row；只加在 CoT 行
       ③ 可达性预算表：syncopate/pipeline/budget_table.py → _audit/v15_w3/budget_table.json
       ④ 行为类 think 探针：scripts/v16/behavior_think_probe.py（契约上下文 + 8B，≥70% 才入库）→ W4 跑
       带宽上沿 0.30 已改（W2 时随 fam 桶一起）
读数   ② 触发可学性探针（scripts/archive/pre_v16_mainline/v15_w3_trigger_probe.py，哈希字符 n-gram + 5 折平衡准确率）：
...
2  行为类 think 探针：scripts/v16/behavior_think_probe.py --n 20 ⇒ _audit/v15_w3/behavior_think_probe.json；
...
4  建库：SYNCOPATE_CONTRACT=v15 SYNCOPATE_THINK=1 python -m syncopate.pipeline.build_sft 2>&1 | tee _audit/v15_w4/build.log
   判据：[同形] 不同形 0 · 份额闸六桶在带（首次实测后把 v13/fam 带宽回填本表）· [CoT-v15] 命中率行入库 ·
        出厂体检 ✅ · syncopate/pipeline/prompt_budget_gate.py --prompt-budget data/sft/v15/train.parquet 零截断
5  压舱不走样：scripts/archive/pre_v16_mainline/v15_r2_migrate.py --scope frozen419 ⇒ 四项全等 419/419
6  新池画像（W3 门槛①）：syncopate/pipeline/budget_table.py 对新 cot 缓存重跑 ⇒ p95 ≤500 字 · 段数 ≤2 · 中文 ≥0.5
7  影子重建判据（如时间允许）：scripts/archive/pre_v16_mainline/run_pipeline_shadow_rebuild.sh 只到 3/6（batches SHA）
```
...
做什么   modal run --detach modal_app/stack_probe.py --steps teacher_diag        # B200 单卡，~15 min
         scripts/v16/teacher_think_diag.py：难例池 20 case × ≤3 步 × 4 样本，max_tokens 4096 只为量真实长度；
...
**E-think · CoT 开/关对任务完成度的 A/B（09-04 Chaoyu 立：「我完全不关心语言，只关心 CoT 和任务正确性有没有关系」；判据跑前注册）**
```
问题     学生底座 Qwen3.6-35B-A3B 在**同一批题、同一采样契约**下，开 thinking 是否比关 thinking 完成得更好？
         不追求 CoT 语言（旗舰模型 CoT 语言本就五花八门，DeepSeek-R1 加语言一致性奖励还掉了点分）。
臂       A think-off（SYNCOPATE_THINK=0，单轮上限 2048）· B think-on（=1，上限 8192；预算不对称是开关设计的一部分，E27 同）
         同一份 `syncopate.train.eval_local`（vLLM 引擎、组内并发），**冻结 EVAL 342 题全跑**（含难例族 112 题），每题 8 样本，temperature=1
         两臂各一台 B200 并行（各臂各目录 /vol/_audit/v16/eval/think_{off,on}.json）；配对按 case_id。
先验     E27（08-19，旧学生 Qwen3-4B / v13）：均分 −0.057（开 think 净减分），但有梯度格子 170→233、卡死 109→60（解锁探索）
预注册判据 / 预测
  J1 总均分配对差 B−A（bootstrap 95% CI）                     预测：|Δ| ≤ 0.05，方向不押
  J2 ★ 区分度子集 = 两臂合并后 case 均分 ∈ [0.2, 0.8] 的题；配对差 B−A 与 CI；子集 n 要 ≥ 30 否则改报"不可判"
                                                            预测：B 高（思考对"一半能做对"的题最有用）
  J3 零梯度构成（σ>0.01 的有梯度题数 / 卡死题数）B vs A         预测：B 有梯度 +≥10% 题（E27 同向）
  J4 有效性：B 臂 truncation=tokens ≤10%；两臂 parse_ok 差 ≤5pp   不满足 = 结论无效，先修预算
  J5 语言：B 臂 think 的 cjk<0.5 占比（预测 ≥90%，即基本英文）· **终答语言**：两臂终答 cjk 中位数之差
     （预测 ≤0.05——英文想不会把中文答带偏；若 B 臂终答明显变英文 = 真代价，要处理）
  J6 成本：每臂墙钟、每样本秒数、步数中位数、B/A 墙钟比（E27 实测 1.4×）
决策规则（跑前写死）  J4 过 ∧（J2 B 显著更好 ∨ J3 有梯度 +≥10%）∧ J1 不显著更差（CI 上界 > −0.05）⇒ **默认开 CoT、不设语言闸**；
                    CoT 蒸馏改为收教师任意语言的思考（撤 cjk 闸 = 重新注册）。否则 CoT 只做 SFT 数据里的按需思考，RL 时 think 关。
```
...
S7 读数（09-04 02:58）🟡 **机制通、语义等 SFT**：B200×2（学生@0、教师+锚@1）· vocab 断言 ✓ · 可训 42.3M · 一个真蒸馏步 kl_chat/tok 0.279、
              ...
              探针 KL 正则量错格式。03:07 复跑：6 步（含跳步 4）400 s 真停 ✓ · final adapter 落盘 ✓ · 真蒸馏步 0（底座）。⇒ S7 真正的冒烟要用 S4 真数据 SFT 的 adapter（--opd-adapter）。
...
**run17（09-04 10:10–10:46，裁定⑭⑮后第一次建库）**
```
CoT 段 ✅ 通了：114 case 全部有思考行（run16 只有 64 且全是旧料）· 采样 6926 条 · 命中 555（**62%**，run16 1%）·
       截断率 **0.0%**（上限 2048）· think 字数 p50/p90/max 209/1088/6212 · 丢弃分布：动作不符 2591 · 空思考 376 · 没写 </think> 271（格式）
       · 预算 45796 内选中 **52 行**，聚合非空 think 252/420 = **60.0%**（恰好压线；行数下限 19 远超）
行为探针  defer 20/20 ✅ · reject 19/20 ✅ · clarify 7/20 🔴（老师想先 campaign.list 再问；欠账照登记，clarify 行不带 think）
定义/闲聊/压舱/L2/L1/家族 全由 27B 重生成 ✅（各自过滤闸全过）
🔴 倒在 CoT 之后：`assert len(t13) == 419`——v13 冻结桶行数写死，v16 派生是 420 ⇒ 按旧单位标定的阈值再一次不报错失效；
   改为「重放行数 == 冻结清单行数」派生判据。另：探针 stale 判据把注释里的旧文件名也数进去（3）⇒ 只数代码行。
   ⇒ run18 从 CoT 缓存（v16_cot_rows.json 114 行）直接进后半段闸：份额/密度/OOV/泄漏/出厂体检/prompt 预算/画廊。
```

**run18（09-04 10:47–10:57，自检前代码，CoT 缓存命中）**：倒在预测的那道——份额闸 l2=9.0% ∉ [10%,17%]。**v16 首次真实份额读数**
...
run24 的其余体检项（量法改为只看监督区后）：六族终答是机器字段句（六族行未注入教师人话）· fam_win 小桶模板重复 · 定义版本 3 ⇒ 5（已改）。
...
⇒ 新增 `syncopate/pipeline/supply_gate.py`（不调教师，只数 SFT 桶供给对照全部下限；runbook `supply` stage + 管线前提检查 data 组）。
...
**W5 起链清单（训练机，09-03 定）**
```
0  python -m syncopate.runtime.seed_demo_data --check   ⇒ 7 条 campaign（含 CMP_7）、安全线/基准/记忆表非空
1  pgrep -f syncopate.runtime.worker 为空（陈旧 worker 抢队列，㉞）
2  bash scripts/v16/exam_chain.sh <合并模型> v15r4 context_v4（四遍；一遍 ~27 min）
   ⚠️ 第一遍跑完先做 W1④ 校准：jsonl 的 think_nonempty 之和 == PG 里 model.thinking 计数
3  syncopate/evaluation/exam_judge.py --context logs/u_route/run_v15r4_r{1..4}_context_v4.jsonl
4  syncopate/evaluation/gate_triage.py 按四遍 judged 读数出表（R5 表：②③⑥ 判、⑤ 只记录、方差前置条件）
...
| 服务 max_model_len | 启动脚本 `--max-model-len` | **24576**（stack_probe.SERVE_MAX_MODEL_LEN，容器内与 rollout_budget 相等断言） | `logs/runtime/start_vllm*.sh` · `scripts/serving/b4_serve_4x.sh` · `scripts/v16/exam_chain.sh`；decider `RUNTIME_MAX_MODEL_LEN` 默认 18432（ctx_cap=−256，生成上限 = min(8192, 余量)） |
...
`prompt_length` + 桶/轴/行为等元列，文本不直接存），`syncopate/pipeline/data_gallery.py --parquet <文件>` 把每桶抽样解码成
