...
> 📦 **历史风险与实验清单，正文中的方框不自动成为当前任务。** 现行说明见
...
> **收官快照（2026-09-02，K 线全部落地后）**：仍为 ⬜/待负向认证的行 = Celery 12 · Redis 7 · PG/Alembic 5 · 接缝 4，
> 共 28 条，加 §5 十条优化候选 = **下一轮实验清单**。它们不挡上线（30 §1 已按条挂账），是 Chaoyu 要的「熟悉队列软件的坑」的教材。
...
|---|---|---|---|---|---|---|---|
| R-01 | 默认只有 RDB 快照 ⇒ 重启丢最近几分钟消息 | 课件 CH3 §4.4"要认真配置持久化"；附录 A §3.3 | 已 dispatched 但未消费的 job 消失；outbox 已标 dispatched ⇒ **没有人再投**（僵尸 queued，H37） | `appendonly yes` + `appendfsync everysec`（`scripts/serving/redis_bootstrap.sh` 已写死并有判据行）；且 sweeper 扫"queued 超龄 ∧ outbox 已 dispatched" ⇒ 告警+人工重投（27 K8 已定） | 关 AOF，投 10 条后 `kill -9 redis` 再起 ⇒ 队列空、10 条 run 停在 queued；sweeper 告警必响 | K3/K8 | 🔶 配置已写，负向认证 ⬜ |
...
| R-06 | Pub/Sub 不持久（订阅者不在线就丢） | 附录 A §5.2 | 若用它做 SSE 唤醒或 outbox nudge，把它当事实就丢事件 | 只当"唤醒"，事实永远回库查（27 K7-3 已定进程内 channel，不用 Redis Pub/Sub；outbox nudge 若用 Redis 也只是优化） | 唤醒通道断开 ⇒ SSE 仍能靠轮询/after 补齐（K7 门槛①） | K7 | ✅ 09-02：SSE 唤醒用进程内 waiter + PG NOTIFY（不用 Redis Pub/Sub）；拔铃测试 test_events_survive_without_the_bell… |
...
| S-02 | 幂等键**从 run_id 推** | `11 §3.9`（validation_errors 那段：client_request_id 从 run_id 推） | 课件 §11.4：同 run 内重试正常，**rerun 换 run_id 就双重扣款** | 写工具 key 绑业务实体+版本位（`campaign_id:动作:v`），只读工具才可带 run_id；K5-6 逐工具登记 key_fn；判据：rerun 同一业务意图 ⇒ 平台去重账本命中 | K5/K6 | 🔶 09-02：loop 路径键=org:tool:hash(args)（无 run_id）✅；写死三步路径（无 decider 时）client_request_id 仍带 run_id，登记保留 |
...
| S-07 | 存档密度：`save_transcript` 是否在"模型点名工具、结果未回"那一档也存 | `agent_loop.py:156/185/205` | 缺这一档 ⇒ 分支 C 无解（课件 CH5 §4.4） | K0 核实；不足则 K5-2 补 | K0/K5 | ✅ 09-02：一轮两档存档 + last/completed_tool_calls（test_loop_k5 断言 tool_use 档存在） |
| S-08 | `resume_after_approval` 只把状态改回 queued，恢复靠 worker 读 checkpoint | `db.py:297` + `agent_loop.py:140` | 若 claim 后没带 `resume=True` 就从头跑（重复模型调用、重复读工具配额） | K0 核实 worker 调用链；判据：审批恢复后模型调用次数 = 剩余轮数 | K0/K5 | ✅ 09-02：任何执行都从最新快照 resume（test_read_only_recovery… 模型调用只 +1、读工具不重调） |
...
| S-19 | 恢复读快照按 `step DESC` 取，会拿到同轮的不完整那档（存档①） | 09-02 假 gate（step 不递增）暴露 | 恢复从"点名前"接 ⇒ 悬空 action 被丢、这一步重做；真 gate 下两档同 key 覆盖故没显形 | 按快照里的消息数 `n` 取最新（`ORDER BY (state->>'n') DESC, id DESC`） | K5 | ✅ 09-02 |
| S-20 | loop 路径上**每个写动作都先过审批**（tier C）⇒ "副作用与记录之间崩溃"的窗口只存在于人已批准后的那次执行 | 09-02 K5 测试首版按"直接写"构造，全部停在 waiting_for_user | 测试若不预埋已批准的审批单，量的是审批流程不是恢复；生产同理：对账/sweeper 的分支 C 场景要按"skip_triggers=True 的执行"来演练 | 测试用 approved 审批单预埋；K8 联测同形 | K5/K8 | ✅ 登记 |
| S-21 | 工具实现崩溃时 `tool_calls` 行停在 running（占了坑、异常穿出、没关行） | 09-02 K6 读 record_tool_call 发现 | K8 sweeper 会把它当"结果未知"标 response_lost 去对账——但它不是半成功，是本地崩了；对账会白查一轮 | 崩溃在 record_tool_call 内关行为 failed/tool_crashed 再抛（分诊表有 tool_crashed 用例） | K6 | ✅ 09-02 |
| S-22 | 课件"写工具 retryable_errors=空集"与我们 09 §2/B-1a 的"带同键重试安全"契约冲突 | 09-02 K6 首版按课件设 {429}，老测试红（upstream_timeout 期望靠键拿回原结果） | 沙盒（训练侧）会重试，runtime 不重试 = runtime 比沙盒少一种行为（09 §1-③ 违反） | 改造：写工具 {429, rate_limited, server_error, timeout} 带键重试；仅本地 client_timeout=未知不重试 | K6 | ✅ 登记（准则五 改造） |
| S-23 | 事件分层 fail-closed：新事件种类忘了在 `event_layer` 登记 ⇒ 前端**静默看不见** | 09-02 K7 设计取舍 | "机制没接上"第七形态的翻版：默认值指向另一件事且不报错——这里是默认 internal | 结构测试 `test_every_emitted_kind_in_code_is_registered_in_the_layer_table`：grep 全部 emit/return 的 kind 必须已登记 | K7 | ✅ 09-02 |
| S-24 | public 流按 seq 补发时跳过 internal 事件 ⇒ 客户端拿到的 Last-Event-ID 可能"跳号" | 09-02 K7 实现 | 若前端/测试断言 id 连续会误红；补发本身无空洞（游标按库里 seq 推进） | 判据只断言"收到的都是 public 且单调递增"，不断言连续；trace 才是全量 | K7 | ✅ 登记 |
| S-25 | sweeper/对账默认全库扫；测试里别的 org 留下的过期 run 会被一起算进计数 | 09-02 K8 测试首跑 | 与 S-14/S-18 同族：判据按"计数"就会被别人污染 | sweep_once/reconcile_once 加 org_id 过滤；判据尽量看自己 run 的事件而不是全局计数 | K8 | ✅ 09-02 |
| S-26 | `updated_at` 触发器让"回填时间做超龄测试"失效（UPDATE 一律被 now() 覆盖） | 09-02 K8 僵尸 queued 测试 | 想造"很久没动"的现场造不出来；生产无影响 | 测试里 DISABLE/ENABLE TRIGGER 包住回填（仅测试）；或超龄判据改看 created_at | K8 | ✅ 登记 |
| S-27 | 假平台的去重账本原来只在进程内存（`_seen_keys`）：worker 重启/换进程后"下游"认不出键 ⇒ 对账无处可查 | 09-02 K8 设计发现 | 课件里最不可控的"下游认不认 key"在我们这本可强可靠，却因为内存态失去了 | platform_ledger 表 + FakeAdPlatform 写穿（ledger 注入）；test_platform_ledger_dedups_across_instances | K8 | ✅ 09-02 |
| S-28 | 假 gate（测试）与真 ActionGate 的契约漂移：每加一个收口方法（stop_requested / budget_exceeded / record_model_usage）老测试的假 gate 就 AttributeError | 09-02 K5/K9 各撞一次 | loop 对 gate 的依赖是隐式鸭子类型；假 gate 不是从同一个基类派生，契约漂移只能靠红测试发现 | 短期：假 gate 显式补方法（已做）；长期：抽一个 `GateProtocol`（typing.Protocol）让假 gate 也过类型检查 | K9 | 🔶 登记 |
| S-29 | `usage_records` 粒度从"每次执行一行"改为"每次模型调用一行"后，写死三步路径仍是每次执行一行（call_index=attempts）——两种粒度并存 | 09-02 K9-3 | /usage 汇总不受影响，但按 call_index 切片分析时两条路径口径不同 | 登记；写死三步路径退役时一并消失（它只在无 decider 的测试/探针里用） | K9 | 🔶 登记 |
| S-30 | Alembic `alembic_version.version_num` 是 varchar(32)：revision 名超长 ⇒ DDL 跑完、版本号写不进去、整段回滚 | 09-02 K10 0008 首跑 | 报错在 UPDATE alembic_version 上，看起来像"迁移写错"，其实是版本表列宽 | revision 名 ≤ 32 字符（`NNNN_k<阶段>_<短名>`）；或 env.py 里 `version_table` 加宽（没做，保持默认） | K10 | ✅ 09-02 |
| S-31 | 回流词表两份真相的风险：K 线若自造 label 枚举，与训练线 cap 名/行为/六族编号就是两套口径 | 09-02 verl-22 回信点出 | 同一症状两种名字 ⇒ 版本切片和回流统计对不上 | label 动态取自 verifier CAPS.names() + contract TERMINAL_SIGNALS + 26 §1.2-S2 六族；reason_code 是 K 线自己的"病因"枚举（训练线没有对应物，登记为 K 线所有） | K10 | ✅ 09-02 |
...
| S-11 | "哪些是写工具"有两份真相：`tool_registry` 的 `kind=="write"` 与 runtime 的 `WRITE_TOOLS` 抄本（`tools.py:41`） | 29 D12 | 登记≠实现的温床：一边加了写工具另一边不知道（08-19 已中过一次） | K6-3 删抄本，权限/幂等/五态全部从 registry 派生；判据 = `WRITE_TOOLS` 符号在 runtime 里不再存在 | K6 | ✅ 09-02：WRITE_TOOLS 从治理表派生；导入时对 REGISTRY 断言完整性与 kind 一致（test_negative_ghost_or_missing_entry…） |
