...
> 📦 **历史设计快照，不代表当前 Runtime。** 现行说明见
> [docs/syncopate/06-RUNTIME.md](../../../syncopate/06-RUNTIME.md)。
...
⇒ **检索的技术选型、阈值标定、两项验收怎么落成 cap、以及 2026-08-16 那次审查
（SFT 桶里 POL/CONF 各 0 条），全部在 `10-rag-retrieval.md`，这里不重复。**

只留一条最容易被重新讨论的结论：**沙盒里不用真向量库**，因为 GRPO 把同一条 case
并发跑 8 遍比分，任何跨 rollout 的检索随机性都会被当成"模型做得不同"记进 advantage。
⇒ 语料逐 case 落在 `readonly_tables` 里，检索是纯函数。
