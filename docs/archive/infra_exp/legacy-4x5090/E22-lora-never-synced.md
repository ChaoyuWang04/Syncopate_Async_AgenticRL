...
### 3.1 离线：分支行为（小模型，不起训练，`scripts/infra/probe_weight_sync_payload.py`）
...
尺子：`scripts/v16/probe_merge_logprob_fidelity.py`（**同引擎、同 dtype、同一批 prompt，只差合并这一步**
...
测试草稿在 **`docs/upstream/OPEN-verl-lora-adapter-sync/`**。跑完 stock verl 已还原。
...
| 🆕 `scripts/infra/probe_weight_sync_payload.py` | 离线复现发送侧两个分支的行为，不占卡 |
