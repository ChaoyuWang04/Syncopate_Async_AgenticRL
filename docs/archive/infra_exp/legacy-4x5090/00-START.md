...
现在的状态   ★★ **全线收官（08-28，三案齐）**：训练线闭环（E31 FP8 定界·占空比 73.4%）
             + serving 线（E32 四卡拓扑·ngram 2.3×）+ 调度层（E33 goodput 64→192 3×·
             膝点移交引擎）；队列为空，只剩行政等待（上游编号×2·DRAFT 等点头）
和主线的关系 两条线共用一个 git 仓库；往来只写根目录 /MAINLINE-INFRA.md（⛔ 禁止信件文档）
```

> **给下一任 infra 负责人的三句话（2026-08-28 留，收官后修订）**：
> ① 这条线**已全线收官**——队列（01）为空，别开新实验；若有人重启，先读 E32 §0 结论卡
> 与各报告的「复活条件」行（PD/内层 FP8/陈旧度都有明确的翻案触发器）；
...
✅ B-4 收官  四卡分布式 serving（E32，08-28 单日 S0–S5 全档）：4 引擎+亲和 router
            （DP×LoRA 上游判死自研补位·扩展 3.66–3.86×）· goodput@SLO=64 并发且膝点在
            编排层 · PD no-go（无物可卸）· **ngram 投机 2.3×/+41% 采纳进生产默认**（冒烟
            实跑）——NARRATIVE 四格全填，**infra 线全线收官**
...
（`scripts/infra/parse_fully_async_timing.py`，单行日志答不了 ⇒ E26 曾因此作废一个 2.12×）；
...
**⑦ 跑与盯**：起跑前过 `scripts/infra/gpu_gate.sh`（⛔ 门禁没过不许起，别用 `;` 放行）；
...
`scripts/infra/check_flash_attn_backward.py`（有轮子**前向全对反向全错**）。
...
写法三条：**大白话**（读者是明天的你）；同一件事**只写一处**、别处只留 `文档号 §节` 互引；
**旧结论错了就地改**（只增不改 ⇒ 两个版本并存，哪个都不准）。

**⑫ 交接双向核对**（Chaoyu 08-29 立，两线通用）：接手读完文档后，**有疑问没疑问都要**
走跨会话通道（`ListAgents` → `SendMessage`）向上一场负责会话问一轮理解确认
（「现场=X、下一步=Y，对吗？有什么没写进文档的坑？」——"读完没问题"最危险，
文档盖不住你误解的那部分）；前任下线联系不上 ⇒ 核对问题+自答写进交接快照，
标「未经前任确认」。**通道**：`ListAgents` 找上一场负责会话的名字 → `SendMessage`
把自己的理解摊开让对方纠（不轮询、不催问）；全文见主线 `docs/syncopate/00-START.md §5⑫`。

**⑬ 没有细到每一步的计划 + 每步的量化验收，就不许动手**（Chaoyu 08-29 立，两线通用）：
开工前写死**每一步做什么 · 产物落在哪 · 用哪个数字判过没过**（阈值 + 读数位置），
判据**跑之前注册**（跑完再定阈值 = 自证）；⛔ 计划里不许留没查清的部分——
「先跑起来再说」等于没有计划，含糊的地方先查清或按⑫问上一任。
全文见主线 `docs/syncopate/00-START.md §5⑬`。

**⑭ 方案定死后用 `/goal` 钉住验收条件**（08-29 立，两线通用）：`/goal <条件>` 设完成条件并
立刻开跑，每轮由小模型判达标没有，没达标自动续跑（`/goal` 看状态 · `/goal clear` 清）。
★★ 评审模型**不跑命令、不读文件，只看对话里已出现的内容** ⇒ **判据必须打印出来**
（否则它在给"我声称做完了"打分 —— 正是守则①「判据行没出现=机制没生效」那个形状）。
条件写法：可测终态 + 怎么证明（哪条命令/读哪个数）+ 不许动的东西 + 轮数上限 +
「不许为了达标放宽判据」。⛔ 方案还模糊时不许用（⑬ 没过 = 只会更快地做错事）。
全文见主线 `docs/syncopate/00-START.md §5⑭`。
...
bash scripts/infra/gpu_gate.sh              # 三条判据全过才许起
```
...
**主线 `../syncopate/04-TRAINING.md`**，两线通用；上面四条是其中的跑中最小集。
...
③ 必跑 scripts/infra/check_flash_attn_backward.py（退出码 0 才算环境可用；反向恒 0 比 nan 更毒）
④ 重建记忆软链（上面那行）
