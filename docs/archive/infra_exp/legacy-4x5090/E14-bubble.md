...
数据 _audit/infra/e14_pg_anatomy.json · 尺子 scripts/infra/analyze_nsys_step.py + 本报告 §4 的洞归因查询
```
...
## 4.10 收官三件（2026-08-21，晋级手续；历史编排 `scripts/archive/legacy_4x5090/run_e14_final3.sh`）
...
**③ compile 微基准 ✅（结论=判死）**：`scripts/infra/bench_compile_update_actor.py`（脱 Ray 单卡，
