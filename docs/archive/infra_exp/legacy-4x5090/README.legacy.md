...
| **E33** 🆕 | [B-5 业务调度层提升](E33-orchestration-goodput.md) | ✅ 08-28 单日收官：**goodput@SLO 64→192（3×三遍复核）**——分账插桩（六桶恒等分解覆盖 99%+）定四瓶颈，五刀（扩池×多进程连落·PG 触发器门铃·router v2 零解析·SLO 优先级）；杀 1/4 worker 零丢单·loadtest 22/22·红线零妥协；C=256 判死于引擎（llm 91%）=膝点移交；**七个翻案**（§6，含 E32"膝点在编排层"被自家判据翻案·priority 无队列无用机理·扩池单独负收益）| **A**（跨线） | ✅ **完成** | — | — |
| **E32** 🆕 | [B-4 四卡分布式 serving 压测](E32-serving-loadtest-4gpu.md) | ✅ 08-28 单日收官（全线最后一案）：4 引擎+亲和 router（DP×LoRA 上游判死自研补位·扩展 **3.66–3.86×**·TTFT P90 −40%）· goodput@SLO=**64 并发**且膝点在编排层（四卡等值证明）· PD **no-go**（98% 命中=无物可卸，chunked 两态拆穿字面 go）· **ngram 投机采纳**（单流 **2.3×**·48 并发 +41%·接受率 64%·50/50 逐字无损）——两项进生产默认冒烟实跑 | **A** | ✅ **完成** | — | — |
...
```
卡间 all-reduce bus bandwidth（NCCL_CUMEM_ENABLE=0，尺子 scripts/infra/probe_allreduce_bw.py）
...
flash-attn: 🆕 **官方 cu13torch2.9 轮子** + PyPI `nvidia-cuda-runtime<=13.2`（补 libcudart.so.13）
    ...
    判据 `scripts/infra/check_flash_attn_backward.py`（含反向）· update_actor 16.78 s vs sdpa 26.01 ⇒ 1.55×
