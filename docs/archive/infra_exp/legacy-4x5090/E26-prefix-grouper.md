...
> 尺子 `scripts/infra/probe_prefix_grouper_speed.py` · 等价性 `scripts/infra/check_prefix_grouper_equivalence.py`
...
> 复现与全部证据：`scripts/infra/repro_pg_dtype.py`（脱 Ray，单轮 ~30 s；三臂 × 5 因素开关 × 4 个探针）。
...
✅ 正式数字来自同尺子 A/B（`scripts/archive/legacy_4x5090/run_e26_ab.sh`，20 global steps/臂 ⇒ 5 行 timing，
...
### 6.6 ✅ 同尺子吞吐 A/B（2026-08-19 13:04，`scripts/archive/legacy_4x5090/run_e26_ab.sh`，判据 `logs/queue_e26ab/AB.done`）
