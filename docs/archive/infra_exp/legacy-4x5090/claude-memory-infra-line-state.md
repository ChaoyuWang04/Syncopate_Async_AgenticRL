...
description: infra 线截至 2026-08-28 的历史状态；当前状态只看 docs/infra_exp/00-START.md 与 01-TASKS.md
metadata:
  node_type: memory
  type: project
...
> **历史记忆，不是当前 infra 队列。** 当前状态只看 `docs/infra_exp/00-START.md` 与
> `docs/infra_exp/01-TASKS.md`；本文只用于回查当时的测量与决策。

> ⛔⛔ **2026-08-18：本条里所有「异步 / 陈旧度 / ESS」相关的结论作废** ——
> 两个基石级 bug（E21 梯度没跨 rank 同步 · E22 权重从没推给 rollout engine）。
> 见 [[two-foundational-bugs-2026-08-18]] 与 `docs/archive/syncopate/pre-consolidation-v16/21-invalidated-numbers.md`。
...
判据是**进程退出 + 显存归还 + 产物目录静默 10 分钟**三条一起查 ⇒ `scripts/infra/gpu_gate.sh`。

⇒ 夜间批：`scripts/archive/legacy_4x5090/run_batch2_gpu.sh`（A14/B2/B3/A5/B12）→ `run_batch3_gpu.sh`（A16/B11/B10）。
...
```
✅ 根因定案（scripts/infra/repro_pg_dtype.py，脱 Ray 单轮 30 s，12 轮 vs 此前 16 轮起训练无果）：
...
⇒ 四格全填·11 §5 已回填·队列空·线收官；仪器 11 件 scripts/serving/b4_*；新机 serving 与旧机
