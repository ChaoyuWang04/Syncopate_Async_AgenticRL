...
**这次是我们自己的 kernel**（`syncopate/train/tilelang_mxfp8.py`）：TileLang 管分块/TMA/流水，
MMA 内层 = 自写设备函数发射 `mma.m16n8k32.kind::mxf8f6f4.block_scale.scale_vec::1X`
（T.import_source 逃生门，绕开 tilelang C++ 校验只认 NVFP4 的限制）。

**三件此前不存在的东西**：
① **缩放 lane 映射硬件反演**（`scripts/infra/probe_mxf8_scale_mapping.cu`，全域唯一指数法）：
...
激活+权重双 8bit）。工具 `scripts/infra/tl_mxfp8_e2e_lmhead.py` + dump 流程见文件头。
...
两型探针（`scripts/infra/mxf8_gemm_limit.cu` / `mxf8_gemm_limit_tma.cu`，全配置 512³ CPU 对拍 ✅，
误差跨配置逐位一致=累加序确定性验证）：
...
探针 `scripts/infra/tl_mxfp8_backward_probe.py`（28,668 真 RL token、真 CE 梯度）：
