> 📦 **已归档（2026-08-19）**：当时的后续结论见
> [`02-DECISIONS.md §4`](../legacy-4x5090/02-DECISIONS.md)；当前状态只看
> `docs/infra_exp/00-START.md`。

# E12 · 权重同步的代价与根因

> ⛔⛔ **2026-08-18 口径更正（[`E22`](../legacy-4x5090/E22-lora-never-synced.md)）**：本报告整份建立在
...
| **问题** | 这笔时间到底花在哪？是传输吗？ |
| **答案** | ⛔⛔ **本报告原来的答案（"既不是传输也不是编排，99.94% 花在处理 132 MB LoRA 上"）已作废** —— 它整份建立在「稳态只推 132 MB」这个**从没量过**的前提上。**实测每次推的是 8,414 MiB 的冻结基座**（[E22 §3.2](../legacy-4x5090/E22-lora-never-synced.md)）。<br>✅ **现在的答案**：修法① 之后 `param_sync` 稳态 **0.974 s**、占一步 **0.8%**；而调大 `sync_every` 省下的钱**根本不在权重同步里** —— **96% 在 `gen`（trainer 等样本）上**（[E08 §5.5](../legacy-4x5090/E08-async-rl.md)） |
| **本报告还剩什么** | ✅ §1–§4.4 的**原始计时数据**（它们量的是真实发生的时间，仍然有效）<br>⛔ §4.5–§4.7 的全部**分析**已移至 [`21-invalidated-numbers.md §5.3`](../../syncopate/pre-consolidation-v16/21-invalidated-numbers.md) |
...
## §4.5–§4.7 —— **已作废，原文移至** [`21-invalidated-numbers.md §5`](../../syncopate/pre-consolidation-v16/21-invalidated-numbers.md)
