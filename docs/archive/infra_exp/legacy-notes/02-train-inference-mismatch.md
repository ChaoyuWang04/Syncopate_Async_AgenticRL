...
2. `rollout_is=sequence` 在 **multi-turn** 场景的语义：`masked_sum` 会把一条轨迹里**所有轮次**的模型 token 的 log_ratio 全加起来。轮次越多、序列越长，S 的方差越大，w 越容易撞上截断上界 C=2.0。**已做纸面量纲分析，见 §8**——结论是 ESS 随 T 指数衰减，这是 Syncopate 的历史研究问题（[23-research-question](../../syncopate/pre-consolidation-v16/23-research-question.md)）。
