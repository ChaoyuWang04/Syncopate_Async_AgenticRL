...
`response_logprobs` 也必须**同步补 0.0**（`:363-364`）保持等长，否则 §4.1 的 `rollout_log_probs` 会和 `response_mask` 错位 —— 这会直接毒化 [02-train-inference-mismatch](02-train-inference-mismatch.md) 里的 TIS 权重计算（log_ratio 逐 token 相减会对错位置）。
