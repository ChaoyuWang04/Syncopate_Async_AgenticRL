...
| **E33** 🆕 | [B-5 业务调度层提升](E33-orchestration-goodput.md) | 施工图已立（08-28 晚，Chaoyu 点单）：把 goodput 膝点 96→**预注册 256**。四大嫌疑（轮询税 0.2/0.25s·连接池 10 条·单进程 GIL·同步记账）全有代码出处；S0 装表分账先行（覆盖率 ≥90% 才许开刀），每步门槛数字不达不进下一步；四红线（幂等/补发/审批/成本）红了即回滚 | **A**（跨线） | ⬜ 施工图已立 | 🔴 唯一在办 | 🔴 |
