...
| # | 任务 | 归谁 | 成本 | 填哪些简历空格 |
|---|---|---|---|---|
| **B-4** | **推理服务真压测 + 优化**（before 已备：24/25 + TPOT/TTFT，`11 §5`）。收尾范围（Chaoyu 08-20 口径 − 多 LoRA〔08-28 撤出交付面〕）：①批调度/chunked prefill 调参 → ②prefix cache×gpu_util 配比 → ③FP8 KV 口径下重记基线；**PD 分离 go/no-go 探针并入本包**（30 min，判据=TTFT/TPOT 差与 KV 走主机内存的账对上）。✅ 已收：占空比 31→**73.4%**（08-28 冒烟同尺采样，两处〔W〕已回填）· 乒乓⑤⑥⑦端到端=冒烟噪声带内不可辨（与 L0 自洽，无回归，验证关账） | infra | 唯一在办 | 剩〔SLO/吞吐〕〔批调度参数〕〔PD 判定〕 |
