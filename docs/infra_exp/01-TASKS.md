...
## 1 · 队首（★ 单任务制延续：**B-5 业务调度层提升**，Chaoyu 08-28 晚点单）

| # | 任务 | 归谁 | 成本 | 填哪些简历空格 |
|---|---|---|---|---|
| **B-5** | **业务调度层提升**（施工图 = [**E33**](E33-orchestration-goodput.md)，B-4 同款纪律：预测写死·逐步门槛数字不达不进下一步）。需求出处 = E32 铁证：膝点 96 与引擎无关（单/四卡等值）。五步：S0 装表分账（覆盖率 ≥90% 才许开刀）→ S1 门铃替轮询（I01 P95 −300ms↑·轮询查询 −80%↑）→ S2 连接池分池（池等待 <5%）→ S3 多进程（C=96 全绿·杀 1 进程零丢单）→ S4 条件步（账本份额 >10% 才做）→ S5 终验（**goodput ≥256** 预注册）。⚠️ 跨线：runtime 归主线，开工已登记 MAINLINE；四红线（幂等/断线补发/审批/成本上限）红了就回滚该步 | infra（跨线） | ~3–4 天 | 〔B-5：膝点/goodput after²〕 |
