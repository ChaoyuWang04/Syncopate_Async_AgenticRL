...
| 🆕 乒乓修理⑤⑥⑦abc（默认开，08-28 profiler 四轮走查） | 逐轮拔榜首、看等待搬家、再拔下一站：⑤PG `convert_padding` 零同步改写（max().item() 172ms/次+nonzero 全消；max_len 走建组 CPU int 表+cumsum/scatter）· ⑥tensordict `.to(device)` 隐藏 synchronize(119ms)→pinned+non_blocking · ⑦a E14-③残余的 2 次批量 tolist（64B DtoH 实测 178ms/次=纯排队）→上卡前 CPU 算长度 · ⑦b verl postprocess 的 unbind→重打包（nested offsets.tolist 144ms）→零同步 jagged 拼接（monkeypatch，回退带响声）· ⑦c 自家 `_to_jagged` 的 as_nested_tensor 构造器内置同步(×2=351ms)→CPU 长度直构 nested_tensor_from_jagged。**三个位等价单测钉死**。终值：>40ms 大额榜只剩 optimizer 合法栅栏 115ms+nested to_copy 88ms（结构性地板）；GPU 空隙 0.64→0.27~0.40s/update_actor（批组合噪声带内，忙闲比 90→94-96%）。★方法论：64B DtoH 的 GPU 侧 0.00ms=等待全是排队；H2D 只等 staging 不排队、DtoH 必排队 | 五个独立开关（=0 对照）+ 3 个位等价测试 + [torch-prof] 账本 |
