...
```
✅ B4写码  E08-c 仪器移到真下发点        完成    三类事件 dispatch/complete/abort + rollout_id 配对，
                                                 尺子 `scripts/analyze_drift.py`；配套修了 Pool.ingest
                                                 把无 reward 的行当 0 分吃进去的静默污染（E08 §8-1）
                                                 ⇒ 只剩「跑一次验证」，已可挂进任一跑
✅ B6写码  动态分池 patch → setup_worker  完成    2026-08-17 已落地并**在跑的 rollouter 进程里
                                                 打出判据行**：`(FullyAsyncRollouter pid=…) [pool] step=…`
                                                 ⇒ 只剩「用采样权重补偿漂移」那一半，顺序仍钉死在 B4 之后
✅ E03成文 NCCL 调优                     完成    只有两个旋钮真有用；ALGO/Simple/BUFFSIZE/NUMA绑定/
                                                 切分次数**全部无效**（都扫过范围）。E03 §3 那张
                                                 「无效」表是这份报告最有用的部分
```
...
✅ A14 真实 ZeRO-3 的分片对齐占比        E18   **已完成 2026-08-17 19:15**：
        真实训练路径上 all_gather **335.5 GB 里 99.9% 的字节**落在 `%16≠0` 的分块上
        （主体 67,287,212 B，%16=12，**每 rank 只差 4 个字节**）；同一跑里 Broadcast
        （不按 rank 切分）**0% 错位**，是最好的对照组。⇒ **因果链闭合**（E18 §12）
🟡 A15 16 字节对齐的上游归属            E18   **草稿已写**（E18 §13）：建议两个都提、
        **先提 PyTorch**（把 per-rank numel 补到 `16/itemsize` 的倍数，改动极小），
        NCCL 那条（Simple kernel 整段放弃向量化）引用它作下游实证。
        ⛔ 更正：我们走的是 **FSDP1**（`FlatParamHandle._get_shard`），不是 FSDP2
        ⚠️ **提之前先做 A16**；且提 issue 是对外动作，要 Chaoyu 点头
🔴 A16🆕 悬崖在**真实分片尺寸**上复现     ~5 min   E18   A15 欠的最后一条证据：
        现有「微基准上补齐就恢复」+「真实路径上确实错位」，**缺「真实尺寸上补 4 字节就恢复」**。
        探针已参数化：`CLIFF_BASE=67287212 CLIFF_OFFS=0,4 …`（已排进 `run_batch3_gpu.sh`）
🟠 B13🆕 ZeRO-3 惩罚的字节账核对            ~30 min  E02   把「100 GB/步 ÷ 3.2 GB/s ≈ 31 s」
        与实测 40 s 的 9 s 缺口坐实（A8 完成后做，顺带回答 ZeRO-2 的 3.42× 是否同源）
```

### 第 1.6 批 🆕 · **夜间 GPU 批**（2026-08-17 排定，等主线让出卡后按序自动跑）

⛔⛔ **前置纪律（用户 2026-08-17 明令）：训练是最高优先级，严禁抢卡。**
必须**同时**满足三条才允许起第一个实验：
① 主线 RL 训练进程退出；② 主线 **eval 及其后续管线全部跑完**（不是日志说完了，
是**产物落地 + 进程退出 + 显存归还**，`scripts/wait_for_gpu.sh` 的那条教训）；
③ 人工（或本窗口）读过产物、确认无后续任务。

| 序 | 任务 | 时长 | 结果 |
|---|---|---|---|
| 1 | ✅ **A14** | 8 min | **99.9% 的 all_gather 字节错位** ⇒ E18 闭环（§12）。同尺子对 Simple/LL128 = **3.23×** |
| 2 | ✅ **B2** · bucket 512 vs 2048 | 18 min | ⛔ **推翻了「bucket 是原因」**：9.04 vs 9.73 s，只差 7.7%（E12 §4.7）⇒ 新问题 **B15** |
| 3 | 🟡 **B3** · 三模式同尺子 | ~40 min | colocate3 **55.28 s/步**（gen 占 48.5%，异步没藏起来）· fully_async **32.57**（B2-512）⇒ **1.70×** |
| 4 | **A5** · nsys 拆解（★ 只能**包住启动**，不能事后 attach ⇒ 挂在第 3 项的某一跑上） | +30 min | B12 与 E14 的共同门槛 |
| 5 | **B12 / E17** · 三次前向（ref 降频 / 共享前向） | ~2 h | 占空比最大的一块（**84.9%**） |
| 6 | **B11** · 拓扑感知放置（trainer 0,1 + rollout 2,3） | ~1 h | 占空比成因④，2+2 才有的问题 |
| 7 | **B10** · 陈旧度节流的代价曲线 | ~2–3 h | 占空比成因②，至今没量过 |
| 8 | **A9** · 4bit MoE 加载路径（时间有余则做） | ~1 h | A2 的前置 |

⚠️ 每一跑的固定动作：**先写预测** → 跑 → `scripts/parse_fully_async_timing.py` 统一口径 →
**删 `checkpoints/grpo/<exp>/global_step_*`**（27 GB/个）→ 回写报告。

### 第 2 批 · 核心数字（贡献力度最大）

★ **B12 提到本批之首**：它是占空比里最大的一块（占步 72%，🆕 **v13 实测 84.9%**）、
**纯计算、不受互联换代影响** ⇒ 在所有「贡献力度」候选里，它是唯一一个价值只会随硬件变好而**上升**的。
...
🐟⑥ A5  E01 一步的时间去哪了        —        E01    ★ B12 的门槛，挂在任意一跑上、几乎零成本
                                                     ⇒ 实际要先于 B12 完成
🟠⑦ B1  E12 最后一刀 + 真的做优化   1–2 天   E12-c  ⚠️🆕 **降级**：bucket 512 下实测占步
                                                     **6.5%（不是 18.8%）**，可回收空间只剩 1/3
                                                     ⇒ **排到 B2/B3 之后**，先拿同尺子分母再决定投不投
                                                     ⇒ 「不在传输上」这个结论仍成立且更硬
...
🔴⑬ B2  缓冲区 512 vs 2048 A/B      ~25 min  E12-e  ⚠️🆕 **提到第 1.6 批第 2 位**：主线 v13-e1
                                                     （bucket 512）实测 param_sync 稳态 **8.43 s**
                                                     vs 本报告 55.8 s（bucket 2048）⇒ 猜想已被野外数据
                                                     强烈支持，**B2 现在的任务是补一个干净分母**
                                                     （只改 bucket，其余全锁死），不是验证猜想
