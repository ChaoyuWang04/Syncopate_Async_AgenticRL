...
### 第 1 批 · 短探测 / gate（约 3 小时，**必须最先**）

★ **两条都排在这里的理由不同**：A6/A7 是**分母**（不先做，后面每次对照都可能白测）；
A1/E04 是 **go/no-go**（挡着以天计的投入）。

```
🔴① A6  E02 三档稳态重测            ~1 h     E02    ★★ **最高优先**。「模型内部并行是净亏损」
                                                     是 A 的筛子②、也是 B 的地基，两条 track
                                                     的简历都引用它。它现在**没有当前数据**
                                                     ⇒ 1 小时决定这句话还能不能说
🔴② A7  E00 满载降频曲线            ~1 h     E00    ★ 分母的分母（4 卡带宽曲线已完成）。
                                                     不先做，后面每一次多卡对照都可能是白测的
🔴③ A1  E07 P2 探针                 ~30 min  E07-P2 ★ go/no-go：verl 能否加载 30B MoE + LoRA
                                                     前向一步。**gate 住 A2 的 2–3 天**
🔴④ E04 TP=2 同 socket 探针         ~30 min  E04🆕  ★ 停放理由已过期（组内 28.8 GB/s）。
                                                     净正就是一条新素材，净负就再停放一次 ——
                                                     两种结果都有价值，30 分钟买断
```
