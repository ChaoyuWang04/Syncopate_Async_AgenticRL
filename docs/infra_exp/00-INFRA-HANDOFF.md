...
```
🔴 A8🆕 集合通信带宽的**分算子口径**        ~30 min  E00   ★ 最要紧的一条：
        A6 的惩罚用 all-reduce 的 25.6 GB/s 完全解释不了，实测 all_gather 只有
        **3.2 GB/s（差 8×）**，按它重算 100 GB/步 ⇒ 31 s，与实测超出的 40 s 吻合。
        ⇒ **E00 的「卡间带宽」不能只有一个数**，要按 all-reduce / all-gather /
          reduce-scatter / broadcast 分别给，否则每次推算都会错。
        （切分粒度已排除：同样 4 GB 切 1 次 vs 576 次，耗时 0.7×，**不是次数的代价**）
