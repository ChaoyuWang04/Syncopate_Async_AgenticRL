...
| ~~3~~ | ✅✅ **B5 第一次通过**（队列 T4）：token 级 IS 那一臂 **+0.101（t=9.3，MDE 0.022）**，三计数 **124 好 / 197 没动 / 22 差**，**不是 reward hacking**（cap 全线下降）。<br>🔴 **两条代价**：该 defer 率 **97%→83%**（`REJ` −0.188、`fabricated_safety_line_cap` +3）· 卡死 **17→34** | infra | — | [E20 §7.5](E20-rl-not-learning.md) | ✅ 结案；⚠️ 序列级那一臂待补跑 |
