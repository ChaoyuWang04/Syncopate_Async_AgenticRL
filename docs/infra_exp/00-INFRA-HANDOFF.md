...
| 序 | 任务 | 时长 | 它把哪个 before 变成 after |
|---|---|---|---|
| 1 | **A14** · 真实 ZeRO-3 的分片对齐占比（+ 强制 `NCCL_PROTO=Simple` 复现 47.94 s 基线） | ~20 min | 闭合 E18 唯一没闭的环 |
| 2 | **B2** · bucket 512 vs 2048 同尺子 A/B | ~25 min | 给 §7.4 那个 6.6× 补干净分母 |
| 3 | **B3** · 三模式同尺子（colocate / one_step_off / fully_async） | ~40 min | 兑现物「三模式同尺子比较」现在是假的 |
