...
✅ 重画像   08-27 七探针全绿（README §6 已改写）：四卡 all-reduce **17.9 GB/s，比旧机低 30%**
            （同 socket 挤一份内存带宽，方向裁决）·三个"可能翻"全复现没翻（3-rank 塌陷 4.3 →
            LL128 14.2 · 16B 悬崖 4.7× · 降频 −0.9% 可忽略）——上游 issue 拿到跨机验证
✅ A3+A4    08-27 一日双收官（[E30](E30-tilelang-nvfp4-gemm.md) 全十三节）：sm120 首个 MXFP8
            块缩放 GEMM（tilelang 543=53% → 裸 CUDA 627=61.2%=消费卡寄存器包络；缩放 lane
            映射硬件反演·XOR 原生布局·负结果三连定界）· 反向 dgrad/wgrad 2.1×/1.7× ·
            **量化温度偏置机理全解剖**（§11：收缩 0.65%≈温度微升 ⇒ RL 采样位正偏置×长度
            复利 ⇒ 序列 IS 判死/token IS 安全/c* 补偿 6×）· **A4：8bit lm_head 前向+反向
            完整 SFT 与 bf16 同带**（§13）· 上游 tilelang 两 DRAFT 包移交（barrier 竞态定价
            15% · MXFP8 支持缺位+原型）
✅ E31 闭环   **训推统一 FP8 六步全闭环（08-27 单日）**：可行域=lm_head（偏置 9× 对消至
            本底 · 400 步长跑三把尺健康 · 配对 +0.109 入带 · 零速度税）；**内层判负定界**
            （激活量化逐层放大异构引擎 hidden 微差，~−1.2e-4/层线性，8 层破门；复活=
            token 级 IS 或同构引擎）——负结果三排除（对齐/字节/噪声）后坐实；
            终审 ±MDE 收紧与否待 Chaoyu 裁定（E31 §1 终审评注）
⬜ 欠的     E31 后续裁定（终审多种子/同位选点加跑与否 · 上游是否发 lm_head-unification
            的消费卡数据点）；CoT 训练支持（产品线）；主机内存带宽未测；
            param_sync 1.02s CPU 侧成分未归因
            CoT 训练支持（产品线）；主机内存带宽未测；param_sync 1.02s CPU 侧成分未归因
