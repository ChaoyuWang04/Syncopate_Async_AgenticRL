...
> ① 这条线已进收尾期，**别开新实验**——队列（01）只剩 B-4 一件，照它施工图打完、把
> NARRATIVE 里剩下的三个〔B-4〕空格用实测数填上，就是合格交接；
...
🔴 唯一在办 **B-4 推理压测收尾包**（01 §1 施工图五步）：填 NARRATIVE 最后三格
            〔SLO/吞吐〕〔批调度参数〕〔PD 判定〕⇒ 全线收官；GPU0 单卡作业
