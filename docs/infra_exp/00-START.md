...
✅ A3+A4    08-27 一日双收官（[E30](E30-tilelang-nvfp4-gemm.md) 全十三节）：sm120 首个 MXFP8
            ...
            **量化温度偏置机理全解剖**（§11：收缩 0.65%≈温度微升 ⇒ RL 采样位正偏置×长度
            ...
            15% · MXFP8 支持缺位+原型）
⬜ 欠的     🔴🔴 **新队首 = 训推部署 FP8 全盘一致**（[E31](E31-unified-fp8.md)，单任务制，
            Chaoyu 08-27 定向；**第 0/1/2 步全绿 + 400 步长跑过闸**：偏置 3.76e-3→4.2e-4
            本底级对消 · 长跑三把尺健康（kl 中位贴线 6.25e-4·ESS 0.902 同 bf16 带）·
            配对 +0.109(t=7.4) 入带 · 零速度税；下一步=第 3 步内层 GEMM 渐进 8bit，
            两个已侦察的硬阻塞（vLLM 内层在 CUDA graph 捕获区 · 逐层 python 量化拖垮
            decode）写在 E31 §2）；
