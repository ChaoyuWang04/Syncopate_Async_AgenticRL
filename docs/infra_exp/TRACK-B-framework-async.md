...
| 「权重同步 = 传输，量小就便宜」 | ⛔ **这条的证据链已作废**：当时以为只推 132 MB，**实际每次推 8.4 GB 冻结基座**（E22）。✅ 修法① 后 `param_sync` **0.974 s / 占步 0.8%**；而真正的代价是**同步打断 rollout**（省下的钱 96% 在 `gen`，E08 §5.5） |
...
1. **权重同步 59.8 s / 4 步**，其中 **99.94% 既不是传输也不是编排**，
   ⛔ 原写「是处理 132 MB LoRA」——**前提错了**，实际推的是 8.4 GB 基座（E22 §3.2）；
...
### 0.7.1 🆕 2026-08-17 夜：**接上了 ≠ 闭环闭上了** —— sampler 跑在 rollout 前面

B2 那一跑（fully_async，12 步）的现场：

```
[pool] step=20…70  {'never_sampled': 659, 'mean_ema_std': 0.0, 'effective_size': 659.0}   ← 全程没动
dispatched.jsonl   576 条 dispatch + 576 条 complete，格式与路径都对
日志行序           第 989 行 = pool 最后一次快照(step=70)   第 993 行 = 第一条 rollout 才刚下发
首条 dispatch → 首条 complete 差 12.1 s
```

**⇒ sampler 在任何一条 rollout 完成之前，就把 70 个 batch 的索引全发完了。**
`Pool.ingest` 没问题（拿这一跑的真实文件离线跑，67 个 case 全部正常吃进、`ema_std` 算得出来），
**是反馈根本还没产生。**

★ **这是「机制在但没接上」的第三形态**：
```
形态① 补丁没打在那个进程       → 判据行在 driver 打了，worker 里没生效
形态② 判据行写的是断言         → 「本轮不生效」把「其实已经行了」盖住
形态③ 🆕 **接上了但时序错开**   → 消费者跑在生产者前面，闭环存在却从未闭合
```

⇒ **对 B6 的直接影响（顺序不用改，但判据要改）**：
1. **短跑验证不了动态分池** —— 判据不能只看启动那行，要看
   `pool/never_sampled` **有没有随步数下降**。（主线 110 步那跑到 pool step 220 时是 589/659，
   确实在降 ⇒ 长跑是生效的。）
2. ★ **更要紧的**：rollouter 预取多深，分池的反馈就滞后多深。
   「量出漂移 → 用采样权重补偿」这条链上，**补偿的滞后量 = 预取深度**，
   而这个量至今没人测过。⇒ **B6 要多加一个观测：sampler 的发出序号 vs 训练消费序号的差。**
   如果预取一整个 epoch，那么"补偿"作用在几百个 batch 之后 —— 那不叫补偿，叫下一轮再说。
...
| 训练一步要把新权重推给推理侧 | 大家默认「LoRA 小所以便宜」 | ✅ **修正后的真相**：框架**根本没在推 LoRA**（推的是 8.4 GB 冻结基座，E22）。接上之后载荷 252 MB、`param_sync` 0.974 s；而同步的真实代价是**打断 rollout 流水线**（E08 §5.5） |
...
| **🔴 B4** | **E08-c 仪器移到真下发点**（case 交给 agent loop 那一刻记，含 `param_version`）**+ 把系统推到触发条件**（调小 `sync_every` / 收紧 `staleness_threshold` / 用更长轨迹）<br>★ **子项 b（别漏）**：`partial_rollout=True` 设了但 37 步**一次没触发**（`partial/*` 全 0）——假设是 rollouter 同步前会排空在飞请求 ⇒ 「abort→续写」这条路从没走过，**又一个没生效的开关**。推到触发条件正好一并验证它 | 现在的「零漂移」是**触发条件没出现**的零差（轨迹中位 17.8 s ≪ 同步周期 296 s），不是结论。而「长度漂移的定量测量」正是 AReaL 明说没做、我们最有资格填的洞 ⇒ **差异化的核心不能是空的** | 🟢 写码<br>🔴 验 | 写码 0.5 天 |
...
| **🔴 B10** 🆕 | **陈旧度节流的代价曲线**：扫 `staleness_threshold` × `sync_every`，画出「占空比 / 吞吐 / 陈旧度 / 任务精度」的权衡面 | ★ **占空比三个成因里，②「陈旧度节流卡住 rollout」至今没有任何实验对着**（§0.55 列了它，但没人打）。现在 `0.1` 是个写死的数，**没人知道它值多少吞吐**。顺带一次把 B7 的 η 换算做了 | 🔴 | 3–4 跑 |
| **🔴 B11** 🆕 | **rollout 的配比与放置**：3+1 独占 vs「4 卡训练、rollout 与某张 trainer 共卡」 | ★ **生成只占一步耗时的 6.3%，那张卡 82.5% 在闲** ⇒ 给它独占一整张卡**可能本身就是错的配比**，而 3+1 是当初拍的、从没测过。这是最直接对着「生成卡在待机」的实验 | 🔴 | 2 跑 |
| **🟢 B12** | **训练侧三次前向** —— ✅ **大头已被 E26 正面打掉**（2026-08-19 同尺子 A/B）：PrefixGrouper 让三次前向从 31.5 → **11.0 s/gstep**（2.86×），端到端 **2.31×**（34.52→14.94），占步 91.3% → **73.9%**。这是 Track B 第一个真正的 **before→after** | 剩余两个方向仍开着：① 砍 `ref`（E17 已定案候选，等 KL 多种子）② E17-C ref 抽样。<br>⚠️ 边界不变：`old_log_prob` **不能降频**（decoupled 下是 proximal anchor，`launch_rl` 注释有读码论证） | 🟢 主体完成 | 欠 B5 |
| ⚪ B8 | E10 · rollout 长尾画像 | 异步收益的理论上界（1.37–2.75×）从哪来 | 🟢 画像 | — |
| ⚪ B9 | E09 · 前缀缓存分片 | 最可能提上游 PR，但不做也不影响这条线成立 | 🔴 | — |

### ⚠️ 3.5.1 一条必须记住的账：**「31%」现在一条成因都还没消掉**

§0.55 把占空比的三个成因列清楚了，但**列出成因 ≠ 解决问题**：

| 成因 | 量级 | 谁在打 |
|---|---|---|
| ① 权重同步 59.8 s / 4 步 | ~~18.8%~~ → 修法① 后 **0.7%** | ✅ E22 修法①（B1 停放） |
| ② `staleness_threshold` 节流 | **没量过** | 🆕 **B10**（之前是空白）；⚠️ E26 之后剂量条件才真正具备 |
| ③ 训练侧三次前向串行 | ~~72%~~ → 91%（修①后）→ **E26 后 73.9%，绝对值 31.5→11.0 s/gstep** | ✅ **E26 PrefixGrouper**（2026-08-19，端到端 2.31×）；剩 ref 方向（E17） |
| ④ 🆕 **配比本身**：生成只占 6.3%，却独占一张卡 | E26 后 gen 占步升到 **26%** ⇒ 配比问题的形状变了 | 🆕 **B11**（要在 E26 之后重新想） |

⇒ **「整机 31% → 〔W〕%」这个 after 是 B1 + B10 + B11 + E13(已落) 共同填的，不是任何一个实验单独填的。**
⇒ ★ 教训：**归因做完会有一种"已经解决了"的错觉。成因列表要标上「谁在打」，空着的格子才显形。**


## 3.6 · ★★★ 覆盖核对：每份 E 报告的「下一步」都要对上一个队列编号

> **纪律（2026-08-16 定，起因是占空比那次）**：
> **观察到一个问题 ≠ 有人在解决它。** 每份 E 报告 §8 的每一条，
> 要么对上 B/A 的一个编号，要么显式写「已结案 / 已停放 + 理由」。
> **对不上的就是空白，而空白只有列出来才看得见。**

| E 报告 §8 的条目 | 归属 | 状态 |
|---|---|---|
| E08-① 仪器移到真下发点 | **B4** | ✅ 已挂 |
| E08-② `partial_rollout` 为什么一次没触发 | **B4 子项 b** | 🆕 此前只写在 E08 里，**队列上没有**，已挂 |
| E08-③ 把系统推到触发条件 | **B4** | ✅ 已挂 |
| E08-⑤ colocate 同机基线 | — | ✅ **已完成**（117.8 s/步） |
| E08-④衍生 · artifact 路径加 `run_id` | — | 🔧 **工程改进不是实验**，挂主线；不加会重复踩「用时间窗圈产物」那个坑 |
| E12-① 8 个步骤分别计时 | — | ✅ **已完成**（99.94% 在第 5 步） |
| E12-② 候选修法（`empty_cache` / buffer 常驻 / KV 是否必须整体释放） | **B1** | ✅ 已挂（B1 定位之后再动，别猜着改） |
| E12-③ 「ESS 刹车能否降频」 | — | ⛔ **已结案：不可行。** `old_log_prob` 在 decoupled 下是 proximal anchor、是损失函数的一部分，降频＝在两个目标函数之间横跳（`launch_rl` 注释有读码论证）。**写在这里是为了防止后人重做** |
| E13-② 剩余 ~5 s/步 的缺口 | **A5（E01）** | ✅ 已挂 —— ⚠️ **A5 是两条 track 共用的**，别因为编号在 A 就以为 B 不需要 |
| E13-③ 上游 DTensor 路径同样在搬冻结基座 | — | 🔧 **可提的上游 PR**，不是本线实验 |
| 占空比成因③ · 训练侧三次前向 72% | **B12** | ✅ **E26 已正面打掉大头**（端到端 2.31×，欠 B5） |
| 占空比成因② · 陈旧度节流的代价 | **B10** | 🆕 同上 |
| 占空比成因④ · 生成侧资源配比 | **B11** | 🆕 同上 |
