...
| **🟠 B12** 🆕 | **训练侧三次前向的必要性**：`update_actor` + `old_log_prob` + `ref` 是**同一批数据的三次前向**，合计占步 **72%**。问：`ref` 能否降频/缓存（KL 系数固定且基座冻结时，参考分布逐步之间变吗）？三次能否共享一次前向的中间结果？ | ★ **占空比成因③ 至今只被侧面蹭到**（E11 切片 4.3% / E13 快照 5.7%），**72% 的大头没有任何实验正面打过**。<br>⚠️ **已知边界**：`old_log_prob` **不能降频**——它在 decoupled 下是 proximal anchor、是损失函数的一部分，降频等于在两个目标函数之间横跳（已读码论证，见 `launch_rl` 注释）。**所以这个实验只能从 `ref` 和"共享前向"两个方向进**。<br>门槛：先做 **A5（E01 拆解）** | 🔴 | 门槛 A5 |
...
| 成因 | 量级 | 谁在打 |
|---|---|---|
| ① 权重同步 59.8 s / 4 步 | 占步 **18.8%** | ✅ **B1** |
| ② `staleness_threshold` 节流 | **没量过** | 🆕 **B10**（之前是空白） |
| ③ 训练侧三次前向串行 | 占步 **72%** | 🆕 **B12**（此前只被 E11/E13 侧面蹭到，**没正面打过**） |
| ④ 🆕 **配比本身**：生成只占 6.3%，却独占一张卡 | — | 🆕 **B11**（之前连成因都没列） |
...
| 占空比成因③ · 训练侧三次前向 72% | **B12** | 🆕 此前**没有任何实验对着**，已挂 |
