...
⇒ **[推断，未验证]**：`merge=True` 之下，vLLM 收到的策略增量很可能是
**"幅度大致对、方向被噪声打乱"**的版本 —— 比"完全没有"好，但**不等于当前策略**。
⇒ 判据（**必须做，见队列 R0-b**）：同一批 prompt，比 **vLLM 的 logprob** 与 **trainer 的 logprob**，
逐 token 看。这正是主线 `19 §5.2` 的 Q2。

⇒ **真正的修法是上游那条 ①**：把 adapter **单独推过去**（vLLM 本来就支持，槽位都建好了），
