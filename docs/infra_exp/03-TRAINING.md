...
| 当前告警 | B02 行为质量问题仍保留；B03 d 仍有长度/轮数 WARN，训推概率差尚待解释。各批原始读数不互相覆盖 |
...
MoE RL 路由一致性由 B03 负责：同一权重、同一 token 和位置，逐层比较 rollout 与 trainer 选中了哪些专家、各自权重多少。先分清模型路由变化、并行重排和浮点误差；再看差异是否传到 logprob 与梯度。路由重放只在官方路径支持时作为独立对照，不预设它一定更好。

35B 的 B03 c 已验真实更新与 adapter；d 又完成同步两端完整字节、版本和原始 token/mask/采样概率回读。trainer 重复前向相等，但与 vLLM 仍有未解释的 logprob 差。另一个两层 full-attention Text MoE 小模型探针已在两张 B200 上通过独立梯度、同状态 AdamW、连续两步和正常 checkpoint 恢复；它不含 35B、GDN、LoRA 或 rollout 引擎。两侧 GDN/MoE 路由和三个性能重复仍未完成，不能提前冻结噪声地板。完整证据与边界见 [B03](../../_audit/infra/B03/REPORT.md)。

身份诊断使用显式 `policy_observer`：外层 engine 调用和内层 optimizer 完成分开计数；FSDP 分片记录自身更新前后的字节指纹，不要求不同分片相等。训推比较保存相同 token/mask 的原始概率，再用同策略重复前向估计噪声；训练仍使用第一次的概率。每轮生成还保留上游实际报告的策略版本，缺失不会猜成 0。此诊断默认关闭，其额外拷贝、文件写入与概率重算不得混进正式测速。当前 FSDP2 没有启用上游 Megatron/VeOmni 的路由重放，不能用后者的开关名充当两侧路由证据。

FSDP 不同 rank 的本地变化张量数可以不同：很小的完整参数会让部分 rank 持有空分片。先看完整/本地形状与字节，再判断是否漏更新；不能把“变化计数相等”当同步正确的替代判据。
...
LoRA、optimizer、adapter 保存/合并和监督 mask 全部兼容；否则 B04 记“不适用”，不为实验手搓 TP。目标 CPU 已取回 Transformers 5.10.4 和 PEFT 0.20.0 的真实 TP/LoRA 保存源码，确认存在接口但尚未执行模型前后向。

SFT 更新的目标是整个累积窗口、所有 DP rank 的监督 token 平均 loss。训练循环先累积 loss 总和，
再由 `token_objective.finish_token_window` 汇总 token 数和梯度、除以实际 token 总数，然后 clip 和更新。
短尾窗口使用实际分母；不能平均各 micro-batch 的均值。日志 loss 和监督 tokens/s 也使用全局口径。
epoch 末对全部可训练参数做字节指纹比较，不再用一个范数代替参数相等。此帮助函数只适用于完整 DP
副本，不可直接用于 TP/FSDP 分片。当前改动已过 CPU 整批/切批、尾窗口和双 rank 数值验收，B200 双卡证据仍待 B04。
...
长度截断使用真实 artifact 的结束原因与剩余预算，不把 `response_length/clip_ratio` 的动态 padding 边界当成配置上限。固定 sync 的覆盖数可由本轮配置核对；异步还要接入消费/预取/丢弃账本，在接通前明确记未测，不能套用 `steps × batch × n` 或把缺读数算成零。
