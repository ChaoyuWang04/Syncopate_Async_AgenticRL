...
> **2026-09-07 定位边界：**原业务训练、选点、评测晋级与完整学习排期停止。下文运行方式和未验项目仅解释保留实现，不是开工指令；新实验按问题单独准入。 当前实验队列见 [Infra TASKS](../infra_exp/01-TASKS.md)。

# Syncopate · 训练与评测

> 本文保留旧业务管线的 SFT、Exam、RL、OPD 实现契约与历史验收；不安排完整学习运行。
> 当前施工顺序只看 [01-TASKS.md](01-TASKS.md)。

## 1. 当前状态

下表是最后验收的 B02 训练结果；旧 T1 状态只看本目录 TASKS，不把新诊断混成一次新的全链通过。
...
当前代码已强制 RL 读取本轮合并 SFT，OPD 读取本轮 RL adapter；B02 已在 B200 证明该产物关系真实成立。这些是保留代码的依赖，独立 base 探针必须另接入口，不能把旧依赖当成新实验前置。

### 原完整学习计划已停止

原 `research/observe` 预算、B07 后完整学习排期和 base/SFT/RL/OPD 业务配对评测已停止；该 profile 尚未实现。新的训练和蒸馏探针按 [Infra TASKS](../infra_exp/01-TASKS.md) 独立选择模型和输入，不复用旧运行授权或身份来冒充新批次。

现有 OPD 实现只蒸馏 `text`，不监督 `think` 或工具协议；学生生成轨迹，教师/anchor 给同一轨迹打概率。全局有效 token 平均 KL、零有效 token 跳步、原始 token/mask 与完整参数字节指纹的实现和验证边界见 [B06 历史记录](../../_audit/infra/B06/REPORT.md)。这些可作为 G5 的线索，不表示目标 CPU/GPU 已验或新入口已经完成。
