...
> 本文是 API、数据库、队列、worker、事件、恢复、发布和 SLO 的保留实现说明。
...
历史模型服务启动脚本不是 B200 正式验收结论。当前环境的进程数、池大小、
队列和模型端点必须在 T2 中按真实机器重新登记。
...
这些项目的状态只在 TASKS 的 T2 维护，本文不保存第二份待办表。
