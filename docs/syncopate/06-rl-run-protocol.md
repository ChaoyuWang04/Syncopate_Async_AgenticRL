...
□ bash scripts/gpu_gate.sh                  证据：退出码 0（抢卡纪律；⛔ 别用 `;` 放行）
□ python scripts/disk_report.py             证据：可用 ≥ 40 GB（收尾一次落 27 GB，M7 丢过 ckpt）
```

### 1.1 静态检查器（2 分钟，⚠️ 必须用项目 `.venv`）

```
□ python scripts/check_pipeline_invariants.py
...
□ 若换过 flash-attn 轮子/机器：python scripts/check_flash_attn_backward.py
...
```
□ 重建前 D 族 / 重切后 L 族：python scripts/check_data_gates.py --batch <批> [--split-dir <切>]
□ SFT↔RL 同构抽样：python scripts/probe_sft_rl_consistency.py     证据：Q5/Q6 双绿
...
□ 三条纪律：不用 lr 1e-4 出候选 · 不用训练分选 ckpt · 守卫必挂：
  bash scripts/rl_guard.sh <日志> <ckpt目录> --kill
□ 长跑（save-freq × 步数 × 27GB > 剩余磁盘时）加挂滚动瘦身：
  bash scripts/rl_ckpt_rolling_prune.sh <ckpt目录> &   # 只留最新全量，其余提 253MB adapter 后删
...
□ python scripts/pool_readout.py <run_dir> --export-triage _audit/triage/<run>/
□ python scripts/candidate_gate.py <run_dir> --strict（晋级闸；probe 跑可跳过）
...
scripts/rl_guard.sh         运行时       盯指标触发就停                     A1 A4 + 2.B + C/D/P 族
...
> 面板程序化建：`python scripts/make_wandb_panels.py`（进仓库不手拖——
...
`python scripts/make_wandb_panels.py`（`--only sft` 可选）。
