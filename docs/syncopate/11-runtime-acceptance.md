...
五个确认的缺口      全部有复现，已变成 5 条 xfail(strict) 测试（§2 逐条展开成任务）
```
...
| R5.7 | RAG 服务（§36） | ⛔ | runtime 侧无任何检索。⇒ `retrieval_empty` 触发器（任务四）和 §19 的「RAG 检索 P95 < 200ms」**都没有承载物** |
...
| **任务四**：给四个降级信号接上生产者 | 🟡 编排最小可用的必然结果 | 大部分归 **M9.4 下半场**（真 Agent Loop）。⚠️ 但**数据成熟度那条不该等** —— 归因延迟是第一性约束，而它现在是一个硬编码的字符串 `"mature"` |
