...
★★ **填门槛的副产品：这张表自己暴露了一个前提不自洽。**
把 TPOT 从「I01 ≤ 5s」反推，算出来是 17.5s —— **超预算三倍多**。
要么 I01 的 5s 太紧、要么简单读意图不该跑满两次生成、要么 4B 在 5090 上的真实
TPOT 远好于反推值。⇒ **压测第一件事是量单流 TPOT 基线**（一次推理就有，不要压测框架）。
**门槛空着的时候，这个矛盾一直藏着。**

**② 五个压测场景的就绪情况**：

```
① 突发 10× 流量    ✅ 并发 8 + org 公平分配已就位（实测 5.5×）
② 模型服务挂掉      ⛔ 没有模型服务（R5.8）—— 这个场景现在没有被测对象
③ 工具超时         ✅ 有（FaultPlan 能注入，且两种超时都建模了）
④ RAG 不可用       ✅ **已有被测对象**（2026-08-17 建完，见 R5.7）：
                   检索服务的 `unavailable` 状态**不会降级成 `no_match`**，
                   并且一律触发降级 —— 这条正是场景④要验的东西
⑤ 单 org 刷爆预算   ✅ 成本闸写前复查 + claim_run 按 org 公平分配（实测小 org 没被饿死）
```

⇒ **场景② 不是"没测"，是"没有被测对象"。**（场景④ 2026-08-17 已经有了。）
模型服务等训练跑完自然就有 ⇒ 压测排在那之后，这条不用现在处理。
