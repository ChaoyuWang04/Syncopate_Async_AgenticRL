...
| **M9 · Runtime 上线态** | ✅ **M9.1–M9.6 施工完成**，⬜ **M9.7 压测五场景未做**。入口 **`09-runtime-handoff.md`** |
...
⇒ 细节全部在 **`09-runtime-handoff.md`**，这里不重复。三条要记住的：
- **幂等是唯一一个"错了就是真金白银"的东西**，判据必须是实测重复投递
- **沙盒是 runtime 的子集，且契约由 runtime 定义**（见 §4-★）
- **PGDATA 放不进 `/workspace`**（mfs 不支持权限位），数据库是派生产物
...
| 事 | 内容 | 成本 |
|---|---|---|
| **M8 验收** | v12 上重训 SFT → 冻结 EVAL 重评 → 量过期检出率/无检索幻觉率 | 几小时 GPU |
...
>
> ⚠️ 数据当前版是 **v12**（v11 的冻结已在 M8 解除），但**v11 的 1370 条输入逐字节相同**
> ⇒ 老基线 `_audit/v11_sft_e1_m2.json` 对它们仍然可比。
>
> **三条会让你在这台机器上直接撞墙的**：
> ① 只能用 DDP，FSDP/TP 是净亏损（§5）
> ② `/workspace` 有 **`df` 看不到的卷配额**，超限静默截断（§6-④）
> ③ `/workspace` **不支持权限位**，所以 PGDATA 只能放本地盘，数据库是派生产物
>    （`bash scripts/pg_bootstrap.sh` 一条命令重建）
>
