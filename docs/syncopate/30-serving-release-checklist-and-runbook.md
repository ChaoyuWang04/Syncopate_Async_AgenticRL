...
> 恢复栏（加法，必须有根因）分开**；每张卡的"第一步看什么"由 `scripts/runbook_queries.py` 逐条真跑
...
| O5 | 做过告警演练（告警真的会触发） | `scripts/runbook_queries.py` 输出 [alert] 行（本机 dev 库上 4 条真触发） | ✓ |
...
| # | 判断句 | 验法 | 状态 |
|---|---|---|---|
| C1 | 九条 SLO 一键打印，读数全部来自表 | `scripts/slo_readout.py`；基线 `_audit/serving_k11/slo_baseline_2026-09-02.txt` | ✓（本机 dev 库上 2 条红，见 §5） |
...
- **第一步看什么**（`scripts/runbook_queries.py` 卡 01）：outbox pending 与最老 pending 秒 · queued 数与最老秒 · "dispatched 但没 pending 也没 started"数 · running 数与活 lease 数。
...
`bash scripts/dr_drill.sh`：干净目录重建 **数据层**（PG initdb 备用端口 → Redis 备用端口 → alembic upgrade head →
