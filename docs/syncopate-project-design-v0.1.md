> **2026-09-07 历史范围说明：**本文仅保留历史材料，不是现行路线、施工指令或可直接发布的成果。原业务造数与完整学习排期已停止；旧目标、机器、数字、PR 状态和待办须按原证据理解，不移入新队列。 当前方向与任务只认 [Infra TASKS](infra_exp/01-TASKS.md)。

# Syncopate · 广告投放全链路 Agent 系统设计文档 v0.1

> 历史设计背景，不是当前状态。旧机器、数据数量和性能假设不再作为默认；现行系统与计算资源见 `syncopate/02-SYSTEM.md`、`syncopate/05-COMPUTE.md`。
...
**★ 2026-08-17 补：原来六条【待定】已全部落地成可判定门槛。**
纪律同 §31.2：**空着的门槛应读作"无法判定"，不是"通过"** ——
一条判据"看起来完整"（有名字、有方向）就很容易被当成已满足。
⇒ 填不出数的，**显式改判为"已知不达标"或从毕业条件里划掉**，不许继续挂着。

| 类别 | 指标 | 门槛 | 这个数怎么来的 |
|---|---|---|---|
| **延迟** | 端到端 **P95**，按意图分 | I01 ≤ **5s** · I07 ≤ **30s** · I09 ≤ **60s** · I11 ≤ **180s** | 沿用本节原有候选值（它们是按"用户愿意等多久"定的，**延迟有绝对业务含义，所以用绝对数**，和 reward 类指标不同）；I07 是新增的中间档（读 + 归因） |
| | 端到端 **P50** | ≤ 对应 P95 的 **40%** | 长尾可以有，但**不能是常态**。P50 贴着 P95 说明慢是普遍的，不是尾部 |
| | 端到端 **P99** | ≤ 对应 P95 的 **2×** | 超过 2× 说明有**一类**请求走了完全不同的路径（重试风暴 / 锁等待），那是结构问题不是长尾 |
| | 模型 **TTFT** P95 | ≤ **800 ms** | 从 I01 ≤ 5s 反推：I01 约 2 次模型调用，首字延迟合计不得吃掉 1/3 预算 |
| | 模型 **TPOT** P95 | ≤ **25 ms/token**（≈40 tok/s） | 同上反推：2 次调用共约 700 输出 token ⇒ 17.5s… ⚠️ **算不过来** ⇒ 见下方「这张表自己暴露的一个矛盾」 |
| | RAG 检索 P95 | < 200ms | 原值保留 |
| | 工具调用 P95（排除 480s 慢工具） | < 2s | 原值保留 |
| **吞吐** | 并发 run 数 | ≥ **8** 条并发且 P95 劣化 ≤ 2× | ✅ **2026-08-17 已具备**（worker 并发 8 + `claim_run` 按 org 公平分配）。冒烟实测 22 条 / 0.96s ≈ 5.5×；⚠️ **真正的判定要等 M9.7**，冒烟不算压测 |
| | 队列积压 | **最老排队时间 ≤ 60s**（= 一个 lease 周期） | 用"等了多久"而不是"排了多少条"：条数受批量影响，时间才是用户感受到的东西 |
| **稳定性** | 崩溃后恢复成功率 | 100% | 原值 |
| | 幂等有效性（重复投递不重复执行） | 100% | 原值。⚠️ 口径补一条：**必须含并发重复投递**，不能只测顺序 |
| | SSE 断线补发成功率 | **100%** | ★ 它是**确定性属性不是统计属性** —— 基于 seq 的重放要么对要么错，没有"95% 对"这种中间态。所以门槛只能是 100% |
| | 长任务（480s 审核）不阻塞其他任务 | 必须 | ✅ **2026-08-17 已具备**：并发化之后一条 run 等外部时会让出，其他 run 照常推进；`claim_run` 的 org 公平分配防止单 org 占满 |
| **成本** | 单任务 token 成本，按意图 | ≤ **该意图 gold 轨迹实测中位数 × 3** | ★ **相对基线而不是绝对数**：各意图的工具链深度差很远（I01 两步 vs I11 十几步），一个绝对数对谁都不对。gold 中位数用 `token_trace` 量，压测前必须先出这个基线 |
| | 单 org 日预算触顶时的降级行为 | 必须有，且**降级不等于失败** | ⚠️ 当前实现是把 run 记成 `failed`。要定：触顶该是"拒绝新 run 但已有 run 跑完"还是"整个 org 停" |
| **压测场景** | ① 突发 10× 流量 ② 模型服务挂掉 ③ 工具超时 ④ RAG 不可用 ⑤ 单 org 刷爆预算 | 全部有明确降级路径 | ⚠️ 这是历史设计门槛；现行验收范围见 `syncopate/07-SERVING.md` |

### ★ 这张表自己暴露的一个矛盾（填门槛的副产品）

把 TPOT 从「I01 ≤ 5s」反推时算不过来：**2 次模型调用 × 约 700 输出 token × 25ms = 17.5s，
已经是 5s 预算的三倍多。** 三种可能，必须先定是哪一种，否则这条门槛写下来就是空的：

```
① I01 的 5s 太紧          —— 它原本是按"查一个指标"定的，而现在 I01 也走完整 agent loop
② I01 不该跑满 2 次生成    —— 简单读意图应该单次、短输出（这是编排要解决的）
③ 4B 模型在 5090 上的真实 TPOT 远好于 25ms —— 那就该实测反填，而不是反推
```

⇒ **压测第一件事就是量 ③**（单流 TPOT 基线，不需要压测框架，一次推理就有）。
量完再决定改 ① 还是改 ②。★ **这正是"门槛写下来才发现前提不自洽"的价值** ——
门槛空着的时候，这个矛盾一直藏着。
...
[ ] ★ premature_decision 命中率 < 2%
[ ] 目标意图（I07/I09/I11）的 reward mean ≥ 基线 + 0.013 且组内 std 仍 > 0
[ ] 熵未塌：探索还在（对比 S1 出口的熵）
[ ] E2E 任务成功率按读/写分桶均达标（三条，见下）
[ ] D1 通用能力未进一步退化
```

**★ 三个【待定】是 2026-08-16 填的，填法本身有讲究**（原来空着 ⇒ M7 那六条里
有三条根本无法判定，却在交接文档里被记成"已验收"）：

**① 门槛一律写成「相对基线 + 配对 MDE」，不写绝对数。**
各意图的基线差得很远（SCALE 0.637 vs CRE 0.889），一个绝对数对谁都不合适。
而 **0.013 是配对最小可检出差异**（`compare.py` 自报，不配对是 0.048，别引用错）——
低于它的"提升"这把尺子根本分辨不出来，写进门槛等于允许拿噪声毕业。

**② 基线取 `_audit/v11_sft_e1_m2.json`**（新机器 + vLLM 引擎跑的 SFT e1）：

```
I07 归因 ATTR   mean 0.842  组内std 0.045   ⇒ 门槛 ≥ 0.855
I09 扩量 SCALE  mean 0.637  组内std 0.218   ⇒ 门槛 ≥ 0.650
I11 素材 CRE    mean 0.889  组内std 0.127   ⇒ 门槛 ≥ 0.902
premature_decision_cap 实测命中 0/198 ⇒ 门槛 2% 是「不许从 0 长出来」，留一条题的余量
```

**③ 读/写分桶的三条，用新补的尺子实测反填**（`eval_local._report_read_write`）。
这把尺子在 M7 验收时**不存在** —— §21 写着「读 ⊥ 写不可合并」，却没有任何代码
按读/写分过桶，所以这一条从头到尾没被判定过。补上之后第一次量就看见了：

```
                  n     reward   满分成功率   cap/次
只读             86     0.909       69%       0.08
含写动作        112     0.722    ★ 20%        0.40
读−写 差值                +0.187
```

> **总均分 0.803 是被读操作撑起来的。真正花钱的那一桶，成功率只有 20%、
> 每次采样撞 0.40 个 cap（读操作的 5 倍）。**
> 这正是 §21 那句「大量读操作会稀释掉写操作的风险」的实物证据。

⇒ 毕业条件三条（896 次写采样下 1σ≈1.3pp，下面的差距都在可分辨范围内）：

```
[ ] 写桶满分成功率 ≥ 30%        （基线 20%）
[ ] 写桶 cap/次   ≤ 0.30        （基线 0.40）
[ ] 读−写 reward 差值 ≤ 0.12    （基线 0.187，要求这条缝收窄而不是靠读操作拉高总分）
[ ] 读桶不得低于 基线 − 0.013   （防"把写做好靠牺牲读"）
```

⚠️ **M7 第一跑对照这套门槛的结果：写桶成功率 20.3%→20.3%（没动）、cap/次 0.40→0.36、
差值 0.187→0.161。** 即「方向对、幅度不够」，与 §2.1 那条
「位移 ≈ lr × 步数，只动了 0.0093%」完全一致。  ⛔(21)
