...
# RDMA 实操学习项目（历史草案，已暂停）

> 本文不是当前任务或运行指南，下面的裸机、购买硬件和其他云平台计划均未启用。旧计划曾限定 Modal CPU/B200；现行资源见 docs/syncopate/05-COMPUTE.md，任务以两份 TASKS 为准。若 Modal 不具备某项底层权限，就记录不适用，不能自行另找机器。

> **目标**：从零成本起步，建立对 RDMA 的**可验证的**工程理解，最终能读懂并有能力改动 NCCL / NVSHMEM / DeepEP 这一层的通信代码。
>
> **面向**：阅读通信原理的项目成员；不作为当前执行指令。
>
...
| 级 | 硬件要求 | 成本 | 能学到 | 优先级 |
|---|---|---|---|---|
| **L0** | Linux 裸机（旧草案前提，当前不提供） | 历史估计，不适用 | verbs 全套语义、QP 状态机、单双边差异、故障模式 | 已暂停 |
...
⚠️ 这是旧裸机前置检查。在当前 Modal 容器中不满足就记录“不适用”，不要据此操作其他服务器或宿主机。
