...
> 承接 [[01-why-verl]] §2 的"两次解耦"——这篇把它落到具体代码行
...
**整个资源调度的全部信息就是这两个 dict。** `resource_pool_spec` 声明有哪些池、每池多大；`mapping` 声明每个角色去哪个池。改部署形态 = 改这两个 dict，算法代码一行不动——这就是 [[01-why-verl]] 说的"解耦①"的物理实现。
...
**注意最后一条**：fully-async **默认开 `bypass_mode=True`**，即 `old_log_probs := rollout_log_probs`，只有两个策略。这和 [[02-train-inference-mismatch]] §3 完全对上——异步模式下 trainer 算力紧张，省掉重算 old_log_prob 的那次 forward 特别划算，而且数据本来就是 off-policy 的，用 rollout logprob 当锚点在语义上更自洽。
