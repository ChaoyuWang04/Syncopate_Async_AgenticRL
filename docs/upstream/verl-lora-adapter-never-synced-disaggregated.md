...
> 同族：[`verl-fsdp-size-1-degenerate-mesh.md`](verl-fsdp-size-1-degenerate-mesh.md)（同一天、同一形状：**配置意图正确，静默走进错误分支**）
...
- [ ] 与同族两份互相引用（`verl-fsdp-size-1-degenerate-mesh.md` /
      `pytorch-fsdp-hybrid-shard-no-grad-sync.md`）—— **同一天、同一形状：
