...
````markdown
Thank you both for taking the time to look at this. You are right that LoRA does not inherently need async — in our case it came from a hardware constraint rather than a preference: on 4 consumer GPUs the trainer and vLLM cannot share a card at our sequence lengths, so trainer and rollout end up on separate devices and run concurrently by construction. The multi-LoRA point makes a lot of sense for the serving setup you describe, and I can see why async would add little there; our situation differs only in that we have a single job in flight, so there is no second tenant to keep the engine busy. I would not want to ask the project to take on async LoRA if you consider it out of scope — that is entirely your call, and a reasonable one. If I may make a smaller request instead: would it be possible for the combination to say something when it is used? At the moment it is accepted silently, training proceeds and the metrics look plausible while the rollout policy never actually changes, which took us a while to spot. A warning or an assert would be more than enough, and I would be glad to rework the PR into that if you would prefer.
````
