...
| A11 | `free_cache_engine=False` 时 `wake_up()` 未被门控，对没睡过的引擎调用是否 no-op | H1a 第一天，实测 |
| A12 | actor 峰值（v8 实测 21.08 GB）在 vLLM 常驻 9.41 GB 后是否还装得下（上限 21.9 GB） | H1a 与 H1b 之间，决定两者能不能分开验收 |
