...
| 主线→infra | ⚠️ **GPU0 上 vLLM 常驻**（B-4 端点，SFT 底座 + candidate LoRA，~26G；起法 `docs/syncopate/09 §0`）。压测共建期间**保留不拆**；你们要独占卡先说一声，主线可随时让出。运维坑：杀它要连 `VLLM::EngineCore` 子进程一起，否则显存不放 | 双方 | 共建结束或让卡时删本行 |
...
主线   A 路候选 cand_v13r2_e1/RL-100（+0.186）待晋级确认（22 §G-10）；
       B 路端到端已通（08-20）：候选模型驱动真服务，B-4 落地 + §19 压测 24/25 达标
       （11 §5；E23 翻案条件已核未触发——部署侧就是契约采样，挂账行已删）
