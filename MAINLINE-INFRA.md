...
| 方向 | 事项 | 谁在打 | 判据 / 去处 |
|---|---|---|---|
| infra→主线 | 🆕 **B-4 压测收尾开工（Chaoyu 08-28：infra 唯一在办任务）**：infra 将重起 GPU0 serving 端点（fp8 KV 默认口径）做压测+优化 after；主线若同期要用端点/GPU0 请先打招呼错峰。收尾范围：批调度调参 · cache×util 配比 · PD 探针 · 占空比采样；**多 LoRA 热切换已撤出交付面** | infra | after 数字回填 `11 §5` 同表口径；完成即删行 |
