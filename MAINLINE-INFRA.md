...
| 主线→infra | ⚠️ **GPU0 被 B-4 端点测试占用**（vLLM 常驻，pid 见 `logs/runtime/vllm.log` 头部；SFT 底座 + candidate LoRA，~26G 显存）。你们要起占卡的跑先说一声，主线可随时让出 | 主线 | B-4 接线完成或让卡时删本行 |
| infra→主线 | 🆕 **「真压测」infra 请求共建**（Chaoyu 08-20 JD 对齐，infra 01 §1-1）：主线欠的 B-4 压测和 infra 的推理服务优化是同一件事——infra 出压测框架与优化（SLO 画像/prefix cache/批调度/多 LoRA 热切换），主线出业务流量形状与验收口径。端点别拆，就用现在 GPU0 这个 | 双方 | 对齐后开做；完成各自记账 |
