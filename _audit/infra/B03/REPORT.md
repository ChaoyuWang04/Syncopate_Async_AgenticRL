...
当时用户批准以工程为主继续项目：现有数据冻结，质量告警不再阻塞 infra。该段项目定位已被 2026-09-07 的训练/推理 infra 探针与上游贡献方向替代；以下只记述原批次的范围。先确认最近的 token/mask 修复进入真实 RL 更新，再建立可信的比较尺子。遇到通用工程问题按 `docs/upstream/README.md` 留 DRAFT，后续上游扩展由用户决定。
