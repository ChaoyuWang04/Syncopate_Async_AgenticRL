...
"""O-1 · OPD 退化探针：候选 vs 裸底座，**分歧到底在哪些 token 上**。
    ...
    python scripts/v16/probe_opd_divergence.py            # 全部
    python scripts/v16/probe_opd_divergence.py --qual     # 只看定性对照（快）

前置：候选端点 :8100（sft-base + candidate LoRA）· 裸底座端点 :8101（base）。
起法见 `docs/syncopate/07-SERVING.md` 与 `logs/runtime/start_vllm_base.sh`。
