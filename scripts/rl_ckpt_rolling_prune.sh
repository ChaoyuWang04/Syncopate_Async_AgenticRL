...
#
#   bash scripts/rl_ckpt_rolling_prune.sh checkpoints/grpo/<run> &
#
...
prune_older() {
  ...
  for gs in "${steps[@]:0:n-1}"; do
    ...
    if [ ! -f "$out/adapter_config.json" ]; then
      ...
      .venv/bin/python scripts/rl_ckpt_to_adapter.py "$gs/actor" --out "$out" || { echo "[rolling] 🔴 提取失败，跳过删除"; continue; }
    fi
