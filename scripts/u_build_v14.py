...
⛔⛔ 本构建器已被 08-29 多样性严审整体否决（24 §7 五实锤：L1「指指」病句 204/250·
    L2 单一话术·标签泄漏·查了不读数），其产物（含盘上 data/sft/v14 的 v14.2 草稿）
    **不许再拿来训练**。v14.2 最终设计在 24 §4-P2（程序造事实·教师穿语言·判据把关），
    动工时本文件将被重写——保留现状只为复现历史数据（v14/v14.1 判定的审计口径）。
