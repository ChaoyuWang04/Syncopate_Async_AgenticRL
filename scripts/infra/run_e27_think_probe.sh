...
#
#   bash scripts/infra/run_e27_think_probe.sh <新SFT的adapter路径>
#
...
bash scripts/infra/gpu_gate.sh || { echo "⛔ 门禁没过，不起"; exit 1; }
...
run_arm() {  # $1=臂名 $2=adapter $3=THINK(0/1) $4=EVAL_EXTRA
  ...
  SYNCOPATE_THINK=$think EVAL_EXTRA="$extra" MODEL=$BASE timeout 7200 \
    bash scripts/infra/eval_parallel.sh "$adapter" "_audit/e27_${name}.json" > "$Q/${name}.log" 2>&1 \
