...
if [ -n "$ck" ]; then
  .venv/bin/python -m syncopate.train.ckpt_to_adapter "$ck" --out models/adapters/e20h_lr1e4_5120 > "$Q/adapter.log" 2>&1
  if [ -d models/adapters/e20h_lr1e4_5120 ]; then
    ...
    MODEL=models/Qwen3-4B-sft-v13-e1 timeout 3600 \
      bash scripts/infra/eval_parallel.sh models/adapters/e20h_lr1e4_5120 _audit/e20h_lr1e4_5120.json > "$Q/eval.log" 2>&1
  fi
