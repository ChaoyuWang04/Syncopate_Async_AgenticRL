...
#   ② 显存    四张卡是否都把显存还回来了
#             ★ `scripts/infra/wait_for_gpu.sh` 开头记着这个坑：日志打完 `[OK] adapter -> …`
...
#
# 用法：
#   scripts/infra/gpu_gate.sh              # 查一次，全过 exit 0
#   scripts/infra/gpu_gate.sh --watch      # 一直查到全过为止（给后台监控用）
#   QUIET_MIN=15 scripts/infra/gpu_gate.sh # 把静默期从默认 10 分钟改成 15
set -uo pipefail
cd "$(dirname "$0")/../.."
