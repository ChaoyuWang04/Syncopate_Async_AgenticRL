...
"""磁盘盘点：把产物按「能不能删」分类，并解释理由。
    ...
    python scripts/tools/disk_report.py            # 只报告，不动任何文件
    python scripts/tools/disk_report.py --plan     # 额外打印可执行的删除命令（仍然不执行）
...
def collect() -> dict[str, list[tuple[str, int, str]]]:
    ...
    if sel:
        out["extract"].append((
            ...
            "`python -m syncopate.train.select_sft_ckpt <out> --keep <名字> --prune`",
        ))
