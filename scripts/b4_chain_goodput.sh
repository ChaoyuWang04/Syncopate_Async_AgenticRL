...
until GPUS=0,1,2,3 bash scripts/gpu_gate.sh >/dev/null 2>&1; do echo "[GP] gate 等 60s" >&2; sleep 60; done
...
bash scripts/b4_stack.sh start 64 || { say "🔴 stack 失败"; exit 1; }
say "loadtest 子集(before)"
timeout 1800 .venv/bin/python scripts/runtime_loadtest.py --skip crash,model_down,cost,sse,idem \
  > "$D/loadtest_before.log" 2>&1 || say "⚠️ loadtest before 非零退出"
say "goodput 阶梯(before)"
timeout 3600 .venv/bin/python scripts/b4_goodput.py --tag before --levels 8,16,24,32,48,64 \
...
bash scripts/b4_serve_4x.sh start 4 affinity -- --max-num-batched-tokens 16384 || { say "🔴 after 拓扑失败"; exit 1; }
bash scripts/b4_stack.sh start 64 || { say "🔴 stack 失败"; exit 1; }
say "loadtest 子集(after)"
timeout 1800 .venv/bin/python scripts/runtime_loadtest.py --skip crash,model_down,cost,sse,idem \
  > "$D/loadtest_after.log" 2>&1 || say "⚠️ loadtest after 非零退出"
say "goodput 阶梯(after)"
timeout 3600 .venv/bin/python scripts/b4_goodput.py --tag after --levels 8,16,24,32,48,64 \
