...
#   menus       case.tool_menu（verifier/routing 用；v15 契约 prompt 一律全量菜单）
...
#   sft-data    data/sft/<DV>/{train,val}.parquet（u_build_v14_5：教师物料 → 六桶 → 全部闸 → 出厂体检 → 隔离复核 → 画廊）
...
#   exam        考场 v4 N 遍 + 判卷 + 三查（scripts/v15_r5_exam_chain.sh，PG/Redis 需已起）
...
stage_sft_data(){ say "[stage sft-data] 建库（教师物料→六桶→全部闸→出厂体检→隔离→画廊）"; need "$SPLIT/sft_cases.json"
  run "SYNCOPATE_TEACHER_LANG_URL=$TEACHER_URL SYNCOPATE_TEACHER_THINK_URL=$TEACHER_URL $PY scripts/u_build_v14_5.py 2>&1 | tee $AUD/$DV/build.log"
...
stage_exam(){ say "[stage exam] 考场 v4 → 判卷 → 三查（PG/Redis 需已起：scripts/pg_bootstrap.sh · redis_bootstrap.sh；smoke=1 遍 40 题）"; need "$MERGED"
  if [ "$PROFILE" = smoke ]; then run "EXAM_PASSES=1 EXAM_LIMIT=40 bash scripts/v15_r5_exam_chain.sh $MERGED ${DV}_smoke context_v4"
  else run "EXAM_PASSES=4 bash scripts/v15_r5_exam_chain.sh $MERGED ${DV}_sft context_v4"; fi; }
