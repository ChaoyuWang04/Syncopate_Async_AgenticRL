...
#
#   bash scripts/serving/pg_bootstrap.sh          # 幂等：已在跑就什么都不做
#   bash scripts/serving/pg_bootstrap.sh --reset  # 推倒重来（会删数据）
#
# ============================================================================
...
(cd "$REPO" && "$PY" -m syncopate.runtime.schema_snapshot --check) || die "schema 与快照不一致——改 schema 只许走迁移链"
