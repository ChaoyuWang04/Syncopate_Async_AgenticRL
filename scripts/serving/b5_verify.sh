...
if ! curl -sf http://127.0.0.1:8100/health >/dev/null 2>&1; then
  ...
  bash scripts/serving/b4_serve_4x.sh start 4 affinity -- --max-num-batched-tokens 16384 \
    --speculative-config '{"method":"ngram","num_speculative_tokens":4,"prompt_lookup_max":4,"prompt_lookup_min":2}' \
    || { say "🔴 fleet 失败"; exit 1; }
fi
bash scripts/serving/b4_stack.sh stop >/dev/null 2>&1
SYNCOPATE_STAGE_TIMING=1 ${STACK_ENV:-} bash scripts/serving/b4_stack.sh start 512 || { say "🔴 stack 失败"; exit 1; }
timeout 3600 .venv/bin/python scripts/serving/b4_goodput.py --tag "$TAG" --levels "$LEVELS" \
...
.venv/bin/python scripts/serving/b5_ledger.py --goodput "$D/gp_$TAG.json" \
