...
用法（先起库、API、worker，见 docs/syncopate/07-SERVING.md）：

    python scripts/serving/runtime_smoke.py                 # 跑固定 query 集，逐条判卷
    python scripts/serving/runtime_smoke.py --burst 20      # 附加：并发提交 20 条看排队

★ 它是**驱动器不是判据的家**：判定“对不对”的口径在 docs/syncopate/06-RUNTIME.md
和 07-SERVING.md，
