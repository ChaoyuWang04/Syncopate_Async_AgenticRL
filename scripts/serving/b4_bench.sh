...
if [ -z "${NO_SERVE:-}" ]; then
  ...
  until GPUS="${SERVE_GPU:-0}" bash scripts/infra/gpu_gate.sh >/dev/null 2>&1; do echo "[B4] gpu_gate 未过，等 60s" >&2; sleep 60; done
...
say "暖机 8 条（并发 4）"
.venv/bin/python scripts/serving/b4_replay.py --base-url "$BASE_URL" --model "$TRACE_MODEL" \
