...
until GPUS=0 bash scripts/infra/gpu_gate.sh >/dev/null 2>&1; do echo "[S3] gate 等 60s" >&2; sleep 60; done

say "臂① chunked on（生产默认）"
serve chunkon || exit 1
.venv/bin/python scripts/serving/b4_pd_probe.py interfere --duration 120 --out "$D/pd_interfere_chunkon.json" || say "🔴 interfere① 失败"
teardown

say "臂② chunked off"
serve chunkoff --no-enable-chunked-prefill || { say "chunked-off 不支持=记档"; SRV=; }
if [ -n "${SRV:-}" ]; then
  .venv/bin/python scripts/serving/b4_pd_probe.py interfere --duration 120 --out "$D/pd_interfere_chunkoff.json" || say "🔴 interfere② 失败"
  teardown
fi

say "带宽探针"
.venv/bin/python scripts/serving/b4_pd_probe.py bw --out "$D/pd_bw.json" || say "🔴 bw 失败"

say "合账"
.venv/bin/python scripts/serving/b4_pd_probe.py account \
...
press() {  # $1=tag
  local tag=$1
  say "$tag greedy 捕获"
  .venv/bin/python scripts/serving/b4_greedy_diff.py capture --out "$D/greedy_$tag.json" > "$D/greedy_$tag.log" 2>&1 || say "🔴 $tag greedy 失败"
  for conc in 1 8 48; do
    say "$tag trace 并发 $conc"
    n=$([ "$conc" = 1 ] && echo 48 || echo 0)
    .venv/bin/python scripts/serving/b4_replay.py --concurrency "$conc" --n "$n" \
...
# 门禁（等 S2/S3 让卡）
until GPUS=0 bash scripts/infra/gpu_gate.sh >/dev/null 2>&1; do echo "[S4] gate 等 60s" >&2; sleep 60; done
...
say "== 无损性判据 =="
.venv/bin/python scripts/serving/b4_greedy_diff.py diff "$D/greedy_base.json" "$D/greedy_ngram.json" | tail -3
echo S4-CHAIN-DONE
