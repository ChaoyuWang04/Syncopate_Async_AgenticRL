...
#
#   bash scripts/dr_drill.sh              # 全程约 30–60s；结束时打印 [dr-drill] RTO=<秒>
#   bash scripts/dr_drill.sh --keep       # 演练后不销毁临时实例
set -euo pipefail
...
PGDATA="$WORK/pgdata" PGPORT="$PGPORT_DR" LOGFILE="$WORK/pg.log" \
  ...
  bash "$ROOT/scripts/pg_bootstrap.sh" | tail -3
...
REDIS_HOME="$ENV" REDIS_DIR="$WORK/redis" REDIS_PORT="$REDIS_PORT_DR" REDIS_PASS=dr-pass \
  bash "$ROOT/scripts/redis_bootstrap.sh" | tail -2
