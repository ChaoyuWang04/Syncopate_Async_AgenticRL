...
_spec = importlib.util.spec_from_file_location("_opd", "scripts/probe_opd_divergence.py")
_opd = importlib.util.module_from_spec(_spec)
sys.modules["_opd"] = _opd
_spec.loader.exec_module(_opd)          # 复用 O-1 验过的分段器（不抄第二份）
    ...
    async with httpx.AsyncClient(base_url=CAND_URL) as c:
        for vname, fields in VARIANTS.items():
            ...
            for msg in CHAT:
                ...
                n_text = sum(1 for lab in _opd.segment_tokens(toks) if lab == "text")
