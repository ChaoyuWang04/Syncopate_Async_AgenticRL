...
用途（K2，2026-09-02，28 P-04/P-05）：
  ...
  python scripts/schema_snapshot.py                 # 打印
  python scripts/schema_snapshot.py --write         # 写 syncopate/runtime/schema.snapshot.txt
  python scripts/schema_snapshot.py --check         # 与快照文件比对，不一致退出码 1（并打 diff）
...
SNAPSHOT = Path(__file__).resolve().parent.parent / "syncopate" / "runtime" / "schema.snapshot.txt"
