"""独立复核器：训练集 parquet 的每一行底题都在它该在的桶里（三桶隔离硬机制的第三层，2026-09-04）。
    ...
    python scripts/check_split_isolation.py data/sft/v16/train.parquet data/sft/v16/val.parquet --pool sft [--split-dir data/splits/v16]

退出码 0 = 全部在桶内；1 = 有越桶行（逐条打印）。派生行按 `source_case_ids` 列核，没有该列按编号后缀反推底题。
被谁调：建库探针（build_v16 判据）· scripts/check_pipeline_invariants.py · 起训自查。
"""
...
sys.path.insert(0, ".")
from syncopate.pipeline.split import DEFAULT_SPLIT_DIR, split_isolation_report  # noqa: E402
