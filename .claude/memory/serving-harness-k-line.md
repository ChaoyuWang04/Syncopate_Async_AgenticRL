...
description: ★K 线（27 serving harness 生产化）现场：09-02 K0–K5 ✅（Celery+Redis 真跑；transition_run 唯一入口；快照式恢复+意图日志+response_lost）；下一步 K6 Tool Runtime；§16 四件已裁（Celery+Redis · PG+Alembic · 快照式恢复 · id 不迁）；坑表 28 · 对照 29；下一步 K1
metadata: 
  node_type: memory
  type: project
